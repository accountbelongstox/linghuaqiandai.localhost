...
function top_html4_upload(input_id,save_dir,dir_append_date,sourceName,top_html4_suffix,max_size,min_size,save_name,fn){
	//cloud_alert(input_id);
	...
	if(document.getElementById(input_id+"_icon")){
    ...
    ajaxFileUpload({
        ...
        callback:function(){
			//cloud_alert(this.responseText);
			temp=this.responseText;
			  	response=eval("("+temp+")");
				if(response.failedList!=''){
                    top_html4_show_failedList(response.failedList,input_id);
                }
				if(response.succeedList!=''){
                    top_html4_show_succeedList(response.succeedList,input_id,response.hiddenValue,save_dir,fn);
                }
...
function top_html4_show_failedList(v,input_id){
    ...
    if(document.getElementById(input_id+"_span")){
...
function top_html4_show_succeedList(v,input_id,hiddenValue,save_dir,fn){
	document.getElementById(input_id).value=hiddenValue;
    if($){
        $("#"+input_id+"_file").attr({
            "data-callvalue":hiddenValue
        })
        if(window.location.search.indexOf('table_add') != -1){
            $("#"+input_id+"_file").next('p').html(hiddenValue)
        }
    }
	//cloud_alert(input_id+"="+document.getElementById(input_id).value);
    ...
    if(save_dir && $){
        ...
        //$(".up_img:has(img)").is("#"+input_id).attr({
        $("#"+input_id+"_img").attr({
            "src":save_dir+hiddenValue
        })
