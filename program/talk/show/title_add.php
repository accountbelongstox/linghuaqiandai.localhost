...
if($type==0){header('location:./index.php?cloud=talk.add');}
$sql="select `title_power` from ".self::$table_pre."type where `id`='".$type."' and `visible`=1";
$r=$pdo->query($sql,2)->fetch(2);
$power=explode('|',$r['title_power']);
if(!in_array($_SESSION['user']['group_id'],$power)){echo self::$language['act_noPower'];return false;}
...
$types_top='<a href="/index.php?cloud=talk.type">'.self::$language['pages']['talk.type']['name'].'</a>';	
$types='';
	
	$sql="select `parent`,`id`,`name` from ".self::$table_pre."type where `id`='".$type."'";
	$r=$pdo->query($sql,2)->fetch(2);
	$types='<a href="/index.php?cloud=talk.title&type='.$r['id'].'">'.$r['name'].'</a>';
	if($r['parent']!=0){
