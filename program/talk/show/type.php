...
$id=$attribute['id'];
if(@$_GET['cloud']=='talk.type'){
	$types_top='<a href="/index.php?cloud=talk.type">'.self::$language['program_name'].'</a>';	
	$types='';
	if(intval(@$_GET['id'])!=0){
		$id=intval(@$_GET['id']);
		$sql="select `parent`,`id`,`name` from ".self::$table_pre."type where `id`='".$id."'";
		$r=$pdo->query($sql,2)->fetch(2);
		$types='<a href="/index.php?cloud=talk.type&id='.$r['id'].'">'.$r['name'].'</a>';
		if($r['parent']!=0){
			$sql="select `parent`,`id`,`name` from ".self::$table_pre."type where `id`='".$r['parent']."'";
			$r=$pdo->query($sql,2)->fetch(2);
			$types='<a href="/index.php?cloud=talk.type&id='.$r['id'].'">'.$r['name'].'</a>'.$types;
		}
	}
...
require($t_path);
if(@$_GET['cloud']=='talk.type'){echo '<div id="visitor_position_reset" style="display:none;"><span id=current_position_text>'.self::$language['current_position'].'</span><a href="/index.php"><span id=visitor_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a>'.$types.'</div>';
}
