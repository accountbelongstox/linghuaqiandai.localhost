...
if($_GET['type']!=0){
	...
	if(!in_array(0,$read_power)){
		if(!in_array(@$_SESSION['user']['group_id'],$read_power)){echo self::$language['no_power_read'];return false;}
	}
}else{
...
$module['batch_operation']='';
	if($manager==@$_SESSION['user']['username']){
		...
		$module['batch_operation']='<div class=cloud_table_batch_operation_div>
...
foreach($r as $v){
  ...
  <td><input type='checkbox' name='visible_".$v['id']."' id='visible_".$v['id']."' value=".$v['visible']."  class='visible' /></td>
	...
	$title='<td><a href="/index.php?cloud=talk.content&id='.$v['id'].'" target="'.$attribute['target'].'" class=title>'.$v['title'].'</a></td>';
	$username='';
	$contents='';
	$visit='';
	$time='';
...
require($t_path);
echo '<div id="visitor_position_reset" style="display:none;"><span id=current_position_text>'.self::$language['current_position'].'</span><a href="/index.php"><span id=visitor_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href=/index.php?cloud=talk.type>'.self::$language['program_name'].'</a>'.$this->get_type_position($pdo,$_GET['type']).'</div>';
