...
if(!in_array('0',$read_power)){
	if(!in_array(@$_SESSION['user']['group_id'],$read_power)){echo '<div align="center" style=" line-height:100px;"><span class=fail>'.self::$language['no_power_read'].'</span></div>';return false;}
}
...
if($manager==@$_SESSION['user']['username']){
...
foreach($r as $v){
	...
	foreach($r2 as $v2){
		...
		$for.="<div class=comment_div id=content_".$v2['id']."><div class=buttons2>".$operation2."</div><div class=comemnt><div class=v>".$v2['content']."</div></div>".'<span class=username>'.$v2['username'].'</span><span class=time>'.get_time(self::$config['other']['date_style']." H:i",self::$config['other']['timeoffset'],self::$language,$v2['time']).'</span></div>';
	}
...
require($t_path);
echo '<div id="visitor_position_reset" style="display:none;"><span id=current_position_text>'.self::$language['current_position'].'</span><a href="/index.php"><span id=visitor_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href=/index.php?cloud=talk.type>'.self::$language['program_name'].'</a>'.$this->get_type_position($pdo,$t['type']).'</div>';
