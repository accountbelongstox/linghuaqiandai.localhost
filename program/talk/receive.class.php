...
class receive{
	...
	function __call($method,$args){
   	...
   	function update_type_sum($pdo,$id,$d=0){
		...
		$sql="select count(`id`) as c from ".self::$table_pre."title where `type`=".$id." and `time`>".$time;
		$r=$pdo->query($sql,2)->fetch(2);
		$day_title_sum=$r['c'];
	...
	function add_content($pdo,$content,$id,$type,$for=0,$email=0){
		...
		$sql="insert into ".self::$table_pre."content (`content`,`title_id`,`username`,`time`,`ip`,`for`,`email`) values ('".$content."','".$id."','".$_SESSION['user']['username']."','".time()."','".get_ip()."','".$for."','".$email."')";
		if($pdo->exec($sql)){
			...
			if($for){
				...
				if($r['email']==1){
					...
					if(is_email($r2['email'])){
						...
						$content=self::$language['content'].':<br />'.$content.'<br /><a href='.get_cloud_path().'index.php?cloud=talk.content&id='.$id.'&current_page='.$end_page.'>'.self::$language['see_details'].'</a>';
						email(self::$config,self::$language,$pdo,'cloud',$r2['email'],$title,$content);	
					}	
				}
			}else{
	...
	function update_content($pdo,$id,$content,$email=0){
		...
		$sql="update ".self::$table_pre."content set `content`='".$content."',`email`='".$email."',`time`='".time()."',`ip`='".get_ip()."' where `id`='".$id."' and `username`='".$_SESSION['user']['username']."'";
		if($pdo->exec($sql)){
