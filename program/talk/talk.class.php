...
//=======================================================================================================
	...
	function get_type_names($pdo,$id,$target,$sum=false){
		...
		foreach($r as $v){
			...
			if($r2['c']!=0){
				$url="./index.php?cloud=talk.type&id=".$v['id'];				
			}else{
				$url="./index.php?cloud=talk.title&type=".$v['id'];	
			}
			if($sum && $r2['c']==0){
				$sum_div='<div class=sum_div>
				<div><span class="day_title_sum_span">'.self::$language['day_title_sum'].'</span><span class="day_title_sum"><span class="v">'.$v['day_title_sum'].'</span></span></div>
				<div><span class="title_sum_span">'.self::$language['title_sum'].'</span><span class="title_sum"><span class="v">'.$v['title_sum'].'</span></span></div>
				<div><span class="content_sum_span">'.self::$language['content_sum'].'</span><span class="content_sum"><span class="v">'.$v['content_sum'].'</span></span></div>
				</div>';
			}else{
				$sum_div='';	
			}
			$names.='<a href="'.$url.'" id="type_'.$v['id'].'" target='.$target.' class=type_a><div class=icon_div><img src="/program/talk/type_icon/'.$v['id'].'.png" /><div class=name>'.$v['name'].'</div></div>'.$sum_div.'</a>';		
		}
		return $names;
	}
	...
	function get_type_position($pdo,$id){
		if(intval($id)==0){return '<a href="/index.php?cloud=talk.type&order='.@$_GET['order'].'">'.self::$language['pages']['talk.type']['name'].'</a>';}
		$position='';
		$sql="select `name`,`parent`,`id` from ".self::$table_pre."type where `id`=$id";
		$r=$pdo->query($sql,2)->fetch(2);
		$position='<a href="/index.php?cloud=talk.title&type='.$r['id'].'&order='.@$_GET['order'].'">'.$r['name'].'</a>';
		if($r['parent']>0){
			$sql="select `name`,`parent`,`id` from ".self::$table_pre."type where `id`=".$r['parent'];
			$r=$pdo->query($sql,2)->fetch(2);
			$position='<a href="/index.php?cloud=talk.type&id='.$r['id'].'&order='.@$_GET['order'].'">'.$r['name'].'</a>'.$position;
		}
		if($r['parent']>0){
			$sql="select `name`,`parent`,`id` from ".self::$table_pre."type where `id`=".$r['parent'];
			$r=$pdo->query($sql,2)->fetch(2);
	...
	function get_content_link($pdo,$id,$title_id=0,$module_config=''){
		...
		$link='./index.php?cloud=talk.content&id='.$title_id.'&current_page='.$end_page."&#content_".$id;
		return $link;		
