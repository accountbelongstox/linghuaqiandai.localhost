...
if($act=='submit_content'){
	...
	if(self::add_content($pdo,$_POST['content'],$id,$type,0,intval($_POST['email']))){
		...
		exit("{'state':'success','info':'<span class=success>".self::$language['success']." <a href=/index.php?cloud=talk.content&id=".$id."&current_page=".$end_page." class=view>".self::$language['view']."</a></span>'}");
	}else{
...
if($act=='submit_comment'){
	...
	if(self::add_content($pdo,$_POST['comment_input'],$id,$type,intval($_POST['for_id']))){
		...
		$html="<div class=comment_div id=content_".$_POST['insret_id']."><div class=buttons2><a href=# onclick=\"return del(".$_POST['insret_id'].")\"  class=del>".self::$language['del']."</a> <span id=state_".$_POST['insret_id']." class=state></span></div><table class=comment_table  cellpadding=0 cellspacing=0><tr><td class=td_1>&nbsp;</td><td class=td_2>&nbsp;</td><td class=td_3>&nbsp;</td></tr><tr><td class=td_4>&nbsp;</td><td class=td_5>".$_POST['comment_input']."</td><td class=td_6>&nbsp;</td></tr><tr><td class=td_7>&nbsp;</td><td class=td_8>&nbsp;</td><td class=td_9>&nbsp;</td></tr></table>".'<span class=username>'.$_SESSION['user']['username'].'</span><span class=time>'.get_time(self::$config['other']['date_style']." H:i",self::$config['other']['timeoffset'],self::$language,time()).'</span></div>';
