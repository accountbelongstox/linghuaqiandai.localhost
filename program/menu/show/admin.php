...
$module['module_name']=str_replace("::","_",$method);
		function admin_menu_get_parent($pdo,$v,$id){
			$sql="select `name`,`id` from ".$pdo->sys_pre."menu_menu where `visible`='1' and `parent_id`=0 and `id`!='$id' order by `sequence` desc";
			...
			foreach($stmt as $v){
				...
				$sql2="select `name`,`id` from ".$pdo->sys_pre."menu_menu where `visible`='1' and `parent_id`=".$v['id']." and `id`!='$id' order by `sequence` desc";
				$r=$pdo->query($sql2,2);
				foreach($r as $v2){
		...
		function admin_menu_get_list($pdo,$language,$parent_id,$module_name){
			...
			$search=trim($search);
			$sql="select * from ".$pdo->sys_pre."menu_menu";
			...
			foreach($r as $v){
				...
				<td><input type='text' name='name_".$v['id']."' id='name_".$v['id']."' value='".$v['name']."'  class='name' style='width:".admin_menu_get_name_width($pdo,$v['id'])."%;' /><a href=# id=icon_".$v['id']." class=icon><img src='/program/menu/icon/".$v['id'].".png'></a></td>
	...
	";	
			$sql2="select count(id) as c from ".$pdo->sys_pre."menu_menu where `parent_id`=".$v['id']."";
		...
		function admin_menu_get_name_width($pdo,$id){
			$sql="select `parent_id`,`id` from ".$pdo->sys_pre."menu_menu where `id`='$id'";
			$v=$pdo->query($sql,2)->fetch(2);
			if($v['parent_id']==0){return 80;}	
			$sql="select `parent_id`,`id` from ".$pdo->sys_pre."menu_menu where `id`='".$v['parent_id']."'";
			$v=$pdo->query($sql,2)->fetch(2);
