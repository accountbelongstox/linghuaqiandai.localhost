...
$module['module_name']=str_replace("::","_",$method);
		...
		function get_list($pdo,$language,$parent,$table_pre,$class){
			global $sum,$module;
			$obj=new $class($pdo);
			$sql="select * from ".$table_pre."article_type where `parent`='$parent'";
	...
	";	
			$sql2="select count(id) as c from ".$table_pre."article_type where `parent`=".$v['id']."";
		...
		function get_name_width($pdo,$id,$table_pre){
			$sql="select `parent`,`id` from ".$table_pre."article_type where `id`='$id'";
			$v=$pdo->query($sql,2)->fetch(2);
			if($v['parent']==0){return 97;}	
			$sql="select `parent`,`id` from ".$table_pre."article_type where `id`='".$v['parent']."'";
			$v=$pdo->query($sql,2)->fetch(2);
