...
$sql="select `src`,`id`,`title`,`type`,`editor`,`sequence`,`visible`,`time`,`visit` from ".self::$table_pre."article";
...
if($_GET['search']!=''){$where=" and (`title` like '%".$_GET['search']."%' or `tag` like '%".$_GET['search']."%' or `src` like '%".$_GET['search']."%' or `content` like '%".$_GET['search']."%')";}
...
foreach($r as $v){
	...
	$img='';
	if($v['src']!=''){
		$img="<br /><a href='index.php?cloud=".$class.".show&id=".$v['id']."&type=".$_GET['type']."&search=".urlencode($_GET['search'])."&order=".$_GET['order']."' target=_blank><img src='/program/article/img_thumb/".$v['src']."' class=thumb_article /></a>";	
	}
	...
	<td><input type='text' name='title_".$v['id']."' id='title_".$v['id']."' value='".$v['title']."'  class='title' />".$img."</td>
