...
//=======================================================================================================
	...
	function get_type_names($pdo,$id,$target,$title=''){
		$names='<a href="/index.php?cloud=article.show_article_list&type='.$id.'" class=parent target='.$target.'>'.$title.'</a>';			
		...
		foreach($r as $v){
			$names.='<a href="/index.php?cloud=article.show_article_list&type='.$v['id'].'" id="article_type_'.$v['id'].'" target='.$target.'>'.$v['name'].'<span class=show_sub>&nbsp;</span></a>';
			...
			foreach($r2 as $v2){
				$names.='<a href="/index.php?cloud=article.show_article_list&type='.$v2['id'].'" id="article_type_'.$v2['id'].'" target='.$target.'>'.$v2['name'].'<span class=show_sub>&nbsp;</span></a>';
				...
				foreach($r3 as $v3){
					$names.='<a href="/index.php?cloud=article.show_article_list&type='.$v3['id'].'" id="article_type_'.$v3['id'].'" target='.$target.'>'.$v3['name'].'</a>';
				}
				$names.='</div>';
			}
			$names.='</div>';
	...
	function get_type_position($pdo,$id){
		if(intval($id)==0){return '<a href="/index.php?cloud=article.show_article_list&order='.@$_GET['order'].'">'.self::$language['pages']['article.show_article_list']['name'].'</a>';}
		$position='';
		$sql="select `name`,`parent`,`id` from ".self::$table_pre."article_type where `id`=$id";
		$r=$pdo->query($sql,2)->fetch(2);
		$position='<a href="/index.php?cloud=article.show_article_list&type='.$r['id'].'&order='.@$_GET['order'].'">'.$r['name'].'</a>';
		if($r['parent']>0){
			$sql="select `name`,`parent`,`id` from ".self::$table_pre."article_type where `id`=".$r['parent'];
			$r=$pdo->query($sql,2)->fetch(2);
			$position='<a href="/index.php?cloud=article.show_article_list&type='.$r['id'].'&order='.@$_GET['order'].'">'.$r['name'].'</a>'.$position;
		}
		if($r['parent']>0){
			$sql="select `name`,`parent`,`id` from ".self::$table_pre."article_type where `id`=".$r['parent'];
			$r=$pdo->query($sql,2)->fetch(2);
