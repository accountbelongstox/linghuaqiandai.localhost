...
if($act=='update'){
	...
	$r=$pdo->query($sql,2)->fetch(2);
	$sql="update ".$pdo->index_pre."page set `head`='".$_POST['head']."',`left`='".$_POST['left']."',`right`='".$_POST['right']."',`full`='".$_POST['full']."',`bottom`='".$_POST['bottom']."',`phone`='".$_POST['phone']."',`layout`='".$_POST['layout']."',`target`='".$_POST['target']."',`tutorial`='".$_POST['tutorial']."' where `id`=$id";
	if($pdo->exec($sql)){
		update_pages_file($pdo,$r['url']);
...
if($act=='go_module_template'){
	...
	$path='./templates/'.$require_login.'/'.$module[0].'/'.$program_config['program']['template_'.$require_login].'/'.$_COOKIE['cloud_device'].'/'.$module[1].'.php';
	if(!file_exists($path)){exit('template '.self::$language['not_exist_file']);}
	header('location:./index.php?cloud=index.template_edit_file&path='.$path);
	exit();
}
