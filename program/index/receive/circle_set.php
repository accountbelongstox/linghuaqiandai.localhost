...
if($act=='add'){
	$sql="insert into ".$pdo->index_pre."circle (`name`,`sequence`,`parent_id`,`center`,`range`,`area_id`,`visible`,`province`,`city`,`county`) values ('$name','$sequence','$parent_id','$center','$range','$area','$visible','".$level_name[1]."','".$level_name[2]."','".$level_name[3]."')";
	if($pdo->exec($sql)){
...
if($act=='update'){
	$sql="update ".$pdo->index_pre."circle set `parent_id`='$parent_id',`name`='$name',`sequence`='$sequence',`center`='$center',`range`='$range',`area_id`='$area',`visible`='$visible',`province`='".$level_name[1]."',`city`='".$level_name[2]."',`county`='".$level_name[3]."' where `id`='$id'";
	if($pdo->exec($sql)){
