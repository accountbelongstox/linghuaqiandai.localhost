...
//暂不启用发送频繁				if(email_frequency($pdo,$_GET['username'])==false){exit("{'errType':'username','errInfo':'".self::$language['email_frequent']."'}");}
				
				$state=email(self::$config,self::$language,$pdo,'cloud',$_GET['username'],$title,$content);
				if($state){
					$state='success';
			...
			if(preg_match(self::$config['other']['reg_phone'],$_GET['username'])){
				...
				$state=sms(self::$config,self::$language,$pdo,'cloud',$_GET['username'],$identifying);
				//$state=true;
				if($state){
					$state='success';
					$info=self::$language['sent'];	
		...
		if($act=='reset'){
			...
			if($v['identifying']==''){exit ("{'errType':'identifying','errInfo':'".self::$language['identifying_expire']."<a href=index.php?cloud=index.resetPassword>".self::$language['reset']."</a>'}");	}
