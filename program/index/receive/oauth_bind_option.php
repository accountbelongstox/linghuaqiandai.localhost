...
if($act=='create'){
	...
	if(isset($_SESSION['user']['username'])){
		exit('<script>window.location.href="../../index.php?cloud=index.user";</script>');	
	}
	...
	$v=$stmt->fetch(2);
	if($v['id']!='' || strtolower($_GET['username'])=='cloud'){$_GET['username'].='_'.get_verification_code(5);}
	$sql="select `id` from ".$pdo->index_pre."user where `username`='".$_GET['username']."' limit 0,1";
	$stmt=$pdo->query($sql,2);
	$v=$stmt->fetch(2);
	if($v['id']!='' || strtolower($_GET['username'])=='cloud'){exit("{'state':'fail','info':'exist'}");	}
	...
	$sql="insert into ".$pdo->index_pre."user (`nickname`,`username`,`password`,`reg_time`,`reg_ip`,`group`,`state`,`manager`,`gender`,`reg_oauth`,`openid`) values ('".$_GET['username']."','".$_GET['username']."','".$_GET['password']."','$time','$ip','".$_GET['group']."',1,$manager,'".$gender."',1,'".$openid."')";
	if($_SESSION['oauth']['icon']!=''){
		...
		if(file_put_contents($path,file_get_contents($_SESSION['oauth']['icon']))){
			$sql="insert into ".$pdo->index_pre."user (`nickname`,`username`,`password`,`reg_time`,`reg_ip`,`group`,`state`,`manager`,`gender`,`reg_oauth`,`icon`,`openid`) values ('".$_GET['username']."','".$_GET['username']."','".$_GET['password']."','$time','$ip','".$_GET['group']."',1,$manager,'".$gender."',1,'".str_replace('./upload/index/user_icon/','',$path)."','".$openid."')";			
		}
