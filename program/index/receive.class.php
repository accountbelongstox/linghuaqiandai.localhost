			...
			if(isset($map[$v['url']])){
				if($i<$one_max){
					...
					if(is_array($map[$v['url']]) && (count($map[$v['url']])>1)){
						...
						$temp='<a href="/index.php?cloud='.$v['url'].'"><img src=/templates/1/'.$program.'/'.$template[$program].'/page_icon/'.$v['url'].'.png /><span>'.$language[$v['url']].'</span><i class="fa fa-angle-down"></i></a>';
						$temp2='';
						...
						foreach($map[$v['url']] as $k2=>$v2){
							$temp3='';
								if(is_array($v2)){
									...
									foreach($v2 as $k3=>$v3){
										if(is_array($v3)){$key3=$k3;}else{$key3=$v3;}
										if(isset($language[$key3])){$temp3.='<li user_color=nv_3><a href="/index.php?cloud='.$key3.'"><img src=/templates/1/'.$program.'/'.$template[$program].'/page_icon/'.$key3.'.png /><span>'.$language[$key3].'</span></a></li>';}
									}
									if($temp3!=''){$temp3='<ul>'.$temp3.'</ul>';}
									if(isset($language[$key2])){$temp2.='<li user_color=nv_2><a href="/index.php?cloud='.$key2.'"><img src=/templates/1/'.$program.'/'.$template[$program].'/page_icon/'.$key2.'.png /><span>'.$language[$key2].'</span><i class="fa fa-angle-right"></i></a>'.$temp3.'</li>';}
									
								}else{
								
									$key2=$v2;
									if(isset($language[$key2])){$temp2.='<li user_color=nv_2><a href="/index.php?cloud='.$key2.'"><img src=/templates/1/'.$program.'/'.$template[$program].'/page_icon/'.$key2.'.png /><span>'.$language[$key2].'</span></a></li>';}
								}
						...
						$temp='<a  href="/index.php?cloud='.$v['url'].'"><img src=/templates/1/'.$program.'/'.$template[$program].'/page_icon/'.$v['url'].'.png /><span>'.$language[$v['url']].'</span></a>';
					}
					
					
					$list[$i]='<li user_color=nv_1>'.$temp.'</li>';	
					$i++;	
				}else{
					...
					if( is_array($map[$v['url']]) && (count($map[$v['url']]) >0) ){
						...
						foreach($map[$v['url']] as $k2=>$v2){
							$temp3='';
								...
								if(isset($language[$key2])){$temp2.='<li user_color=nv_3><a href="/index.php?cloud='.$key2.'"><img src=/templates/1/'.$program.'/'.$template[$program].'/page_icon/'.$key2.'.png /><span>'.$language[$key2].'</span></a></li>';}
						}
						$temp.=$temp2;	
						$list[$one_max].='<li user_color=nv_2><a href="/index.php?cloud='.$v['url'].'"><img src=/templates/1/'.$program.'/'.$template[$program].'/page_icon/'.$v['url'].'.png /><span>'.$language[$v['url']].'</span></a><ul>'.$temp.'</ul></li>';	
					}else{
						$other.='<li user_color=nv_2><a href="/index.php?cloud='.$v['url'].'"><img src=/templates/1/'.$program.'/'.$template[$program].'/page_icon/'.$v['url'].'.png /><span>'.$language[$v['url']].'</span></a></li>';
					}
...
//----------------------------------------------------------------------------------------------------------------------------------more end					
		...
		//var_dump($other);
		if($other!=''){$other='<li user_color=nv_2><a>'.self::$language['composite'].'</a><ul>'.$other.'</ul></li>';}
		if(!isset($list[$one_max])){$list[$one_max]='';}
		$list[$one_max]=$other.$list[$one_max];
		if($list[$one_max]!=''){$list[$one_max]='<li user_color=nv_1><a href="#"  id=more_admin>'.self::$language['more'].'<i class="fa fa-angle-down"></i></a><ul>'.$list[$one_max].'</ul></li>';	}
		
		
		$str='<li user_color=nv_1><a href="/index.php">'.self::$language['home'].'</a></li>';
		foreach($list as $k=>$v){
			$str.=$v;	
		}
		//echo $str;
		$sql="update ".$pdo->index_pre."group set `map`='".$str."' where `id`=".$group_id;
		$pdo->exec($sql);
	}
//====================================================================================================================================update_admin_nv end
...
//====================================================================================================================================update_navigation start
	...
	//----------------------------------------------------------------------------------------------------------------------------------more end					
		...
		if($list[$one_max]!=''){
			...
			$list[$one_max]='<li><a href="#"  id=more_admin><img class="receive_nav_img" src="'.$icon.'" /><span>'.self::$language['more'].'</span><i class="fa fa-angle-down"></i></a><ul>'.$list[$one_max].'</ul></li>';	
		}
