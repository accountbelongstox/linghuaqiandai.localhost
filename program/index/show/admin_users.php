...
$sql="select `id`,`icon`,`nickname`,`username`,`real_name`,`email`,`gender`,`birthday`,`education`,`married`,`annual_income`,`reg_time`,`last_time`,`login_num`,`money`,`introducer`,`group`,`state`,`profession`,`blood_type`,`height`,`weight`,`phone`,`home_area`,`current_area`,`manager`,`address`,`cumulative_credits`,`geolocation`,`openid` from ".$pdo->index_pre."user";
...
if(@$_GET['group']!=''){
	...
	if($r2['id']==$_SESSION['user']['group_id']){$sql2.=" and `id`='".$_SESSION['user']['id']."'";$manager_list='';}
	//echo $sql2;
	$r2=$pdo->query($sql2,2);
	foreach($r2 as $v){
...
$sql2="select count(id) as c from ".$pdo->index_pre."user where `group`='".$_SESSION['user']['group_id']."'";
$r2=$pdo->query($sql2,2)->fetch(2);
if($r2['c'] == 1 || $_SESSION['user']['group_id']==1){
	$groups=index::get_group_sub_ids($pdo,$_SESSION['user']['group_id']);	
	$groups=trim($groups,",");
	if($groups==''){$groups=0;}
	$where.=" and `group` in ($groups) or `user_group` in (".$_SESSION['user']['id'].")";
	//echo $groups;
}else{
	$ids=index::get_user_sub_ids($pdo,$_SESSION['user']['id']);	
	$ids=trim($ids,",");
	if($ids==''){$ids=0;}
	$where.=" and `id` in ($ids)";
	//echo $managers; 
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	$sum_sql=$sql.$where;
	$sum_sql=str_replace(" `id`,`icon`,`nickname`,`username`,`real_name`,`email`,`gender`,`birthday`,`education`,`married`,`annual_income`,`reg_time`,`last_time`,`login_num`,`money`,`introducer`,`group`,`state`,`profession`,`blood_type`,`height`,`weight`,`phone`,`home_area`,`current_area`,`manager`,`address`,`cumulative_credits`,`geolocation`,`openid` "," count(id) as c ",$sum_sql);
...
foreach($r as $v){
	if($v['geolocation']!=''){
		...
		$v['geolocation']='<a class="fa fa-map-marker" href="/include/core/get_point.php?id=position&point='.$temp[1].','.$temp[0].'" target="_blank"></a>';	
	}
	if($v['openid']!=''){	
		$v['openid']='<a href="/index.php?cloud=weixin.dialog&wid='.self::$config['web']['wid'].'&openid='.$v['openid'].'" target=_blank class=openid></a>';
	}
	if($_COOKIE['cloud_device']=='pc'){
		$list.="<tr id='tr_".$v['id']."'>
		<td><input type='checkbox' name='".$v['id']."' id='".$v['id']."' class='id' /></td>
		<td><a class=icon href='/upload/index/user_icon/".$v['icon']."'><img src='/upload/index/user_icon/".$v['icon']."' /></a></td>
		<td>".$v['username'].$v['geolocation'].$v['openid']."<br />".$v['real_name']."<br />".$v['email']."<br />".$v['phone']."</td>
		
		<td><a class='login_num' href='index.php?cloud=index.edit_user_group&id=".$v['id']."' target=_blank>".self::get_group_name($pdo,$v['group'])."</a></td>
		<td><a href=/index.php?cloud=index.credits_admin&search=".urlencode($v['username'])." target=_blank>".$v['cumulative_credits']."</a></td>
		<td><a href=/index.php?cloud=index.money_log_admin&username=".urlencode($v['username'])." target=_blank>".$v['money']."</a><br />
			<a href=/index.php?cloud=index.user_recharge&id=".$v['id']." target=_blank>".self::$language['recharge']."</a><br />
			<a href=/index.php?cloud=index.user_debit&id=".$v['id']." target=_blank>".self::$language['debit']."</a>
		</td>
		<td>".get_time(self::$config['other']['date_style'],self::$config['other']['timeoffset'],self::$language,$v['reg_time'])." <a href=/index.php?cloud=index.login_log&id=".$v['id']." target=_blank class=login_num>".$v['login_num']."</a></td>
	...
	<a href='index.php?cloud=index.admin_edit_user&id=".$v['id']."' class=edit target='_blank'>".self::$language['edit']."</a> 
...
";	
		...
		<td><a class=icon href='/upload/index/user_icon/".$v['icon']."'><img src='/upload/index/user_icon/".$v['icon']."' /></a></td>
		<td class=u_info>
		<div><span class=u_label>".self::$language['username'].":</span><span class=u_value>".$v['username'].' '.$v['geolocation'].$v['openid']."</span></div>
		<div><span class=u_label>".self::$language['real_name'].":</span><span class=u_value>".$v['real_name']."</span></div>
		<div><span class=u_label>".self::$language['user_group'].":</span><span class=u_value><a href='index.php?cloud=index.edit_user_group&id=".$v['id']."' target=_blank>".self::get_group_name($pdo,$v['group'])."</a></span></div>
		...
		<td><a href=/index.php?cloud=index.cumulative_credits&search=".urlencode($v['username'])." target=_blank>".$v['cumulative_credits']."</a></td>
		<td><a href=/index.php?cloud=index.money_log_admin&username=".urlencode($v['username'])." target=_blank>".$v['money']."</a><br /><a href=/index.php?cloud=index.user_recharge&id=".$v['id']." target=_blank>".self::$language['recharge']."</a><br /><a href=/index.php?cloud=index.user_debit&id=".$v['id']." target=_blank>".self::$language['debit']."</a></td>
		<td>".get_time(self::$config['other']['date_style'],self::$config['other']['timeoffset'],self::$language,$v['reg_time'])." <a href=/index.php?cloud=index.login_log&id=".$v['id']." target=_blank class=login_num>".$v['login_num']."</a></td>
		<td><select id='user_state_".$v['id']."' name='user_state_".$v['id']."'>".get_user_state(self::$language,$v['state'])."</select></td>
		<td class=operation_td>	<a href='#' class=submit onclick='return update(".$v['id'].")'>".self::$language['submit']."</a> 
	<a href='index.php?cloud=index.admin_edit_user&id=".$v['id']."' class=edit target='_blank'>".self::$language['edit']."</a> 
...
$module['filter'].='<br/><span class="m_label">'.self::$language['user_group'].'</span><span class="option"><select id=group name=group><option value="">'.self::$language['all'].'</option>'.index::get_group_select($pdo,'-1',$_SESSION['user']['group_id']).'</select> &nbsp; &nbsp; &nbsp; '.self::$language['parent'].'<select id=manager name=manager>'.$manager_list.'</select> </span>';
...
$module['bulk_action']='';
if($sum>0){
	$module['bulk_action']=self::$language['bulk_action'].$sum.self::$language['a'].self::$language['user'].': <a href="'.$module['action_url'].'&act=export_csv" target="_blank" class=export>'.self::$language['export'].'csv</a> <a href="'.$module['action_url'].'&act=site_msg" target="_blank" class=site_msg>'.self::$language['send'].self::$language['site_msg'].'</a> <a href="'.$module['action_url'].'&act=email_msg" target="_blank" class=email_msg>'.self::$language['send'].self::$language['email_msg'].'</a>';
	
	$module['selected_action']='<li><a href="'.$module['action_url'].'&act=send_wxmoney" target="_blank" class=send_wxmoney>'.self::$language['pages']['index.send_wxmoney']['name'].'</a></li> <li><a href="'.$module['action_url'].'&act=export_csv_selected" target="_blank" class=export>'.self::$language['export'].'csv</a></li> <li><a href="'.$module['action_url'].'&act=site_msg_selected" target="_blank" class=site_msg>'.self::$language['send'].self::$language['site_msg'].'</a></li> <li><a href="'.$module['action_url'].'&act=email_msg_selected" target="_blank" class=email_msg>'.self::$language['send'].self::$language['email_msg'].'</a></li>';
