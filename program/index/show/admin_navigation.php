...
$module['module_name']=str_replace("::","_",$method);
		function admin_navigation_get_parent($pdo,$v,$id){
			$sql="select `name`,`id` from ".$pdo->index_pre."navigation where `visible`='1' and `parent_id`=0 and `id`!='$id' order by `sequence` desc";
			...
			foreach($stmt as $v){
				...
				$sql2="select `name`,`id` from ".$pdo->index_pre."navigation where `visible`='1' and `parent_id`=".$v['id']." and `id`!='$id' order by `sequence` desc";
				$r=$pdo->query($sql2,2);
				foreach($r as $v2){
					if($temp==$v2['id']){$selected='selected';}else{$selected='';}
					$module['parent'].="<option value='".$v2['id']."' $selected >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$v2['name']."</option>";
				}	
			}
			return $module['parent'];			
		}
		
		function admin_navigation_get_list($pdo,$language,$parent_id,$module_name){
			global $sum;
			$visible=@$_GET['visible'];
			$open_target=safe_str(@$_GET['open_target']);
			$search=safe_str(@$_GET['search']);
			$search=trim($search);
			$sql="select * from ".$pdo->index_pre."navigation";
			...
			if($sum==0){
				...
				$sum_sql=str_replace("_navigation and","_navigation where",$sum_sql);
				$r=$pdo->query($sum_sql,2)->fetch(2);
				$sum=$r['c'];
			}
			$sql=$sql.$where.$order;
			$sql=str_replace("_navigation and","_navigation where",$sql);
			//echo($sql);
			//exit();
			$r=$pdo->query($sql,2);
			$list='';
			foreach($r as $v){
				...
				".admin_navigation_get_parent($pdo,$v['parent_id'],$v['id'])."
				</select></td>
				<td><input type='text' name='name_".$v['id']."' id='name_".$v['id']."' value='".$v['name']."'  class='name' style='width:".admin_navigation_get_name_width($pdo,$v['id'])."%;' /><a href=# id=icon_".$v['id']." class=icon><img class='nav_index_img' src='/program/index/navigation_icon/".$v['id'].".png'></a></td>
	...
	";	
			$sql2="select count(id) as c from ".$pdo->index_pre."navigation where `parent_id`=".$v['id']."";
			$r2=$pdo->query($sql2,2)->fetch(2);
			if($search==''){
				if($r2['c']>0){$list.=admin_navigation_get_list($pdo,$language,$v['id'],$module_name);}
			}
			}
			return $list;
		}
		
		function admin_navigation_get_name_width($pdo,$id){
			$sql="select `parent_id`,`id` from ".$pdo->index_pre."navigation where `id`='$id'";
			$v=$pdo->query($sql,2)->fetch(2);
			if($v['parent_id']==0){return 80;}	
			$sql="select `parent_id`,`id` from ".$pdo->index_pre."navigation where `id`='".$v['parent_id']."'";
			$v=$pdo->query($sql,2)->fetch(2);
		...
		function admin_navigation_get_visible_filter($pdo,$language){
		...
		function admin_navigation_get_open_target_filter($pdo,$language){
		...
		function admin_navigation_get_parent_filter($pdo,$language){
			$list="<option value='-1'>".$language['parent']."</option>";
			$list.="<option value='' selected>".$language['all']."</option>";
			$parent_id=@$_GET['parent_id']?@$_GET['parent_id']:0;
			$list.=admin_navigation_get_parent($pdo,$parent_id,0);
		...
		$module['parent']=admin_navigation_get_parent($pdo,'-1',0);
		$_SESSION['token'][$method]=get_random(8);$module['action_url']="/receive.php?token=".$_SESSION['token'][$method]."&target=".$method;
		$sum=0;
		$module['list']=admin_navigation_get_list($pdo,self::$language,0,$module['module_name']);
		...
		$module['filter']=admin_navigation_get_visible_filter($pdo,self::$language);
		//$module['filter'].=admin_navigation_get_parent_filter($pdo,self::$language);
		$module['filter'].=admin_navigation_get_open_target_filter($pdo,self::$language);
