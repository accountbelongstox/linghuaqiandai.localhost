...
function admin_circle_get_list($pdo,$language,$parent_id,$module_name){
	...
	foreach($r as $v){
		...
		<td class=name_td><a href=# id=icon_".$v['id']." class=icon><img src='/program/index/circle_icon/".$v['id'].".png'></a><input type='text' name='name_".$v['id']."' id='name_".$v['id']."' value='".$v['name']."'  class='name' style='width:".admin_circle_get_name_width($pdo,$v['id'])."%;' /></td>
	  <td><input type='text' cloud_type='map' name='center_".$v['id']."' id='center_".$v['id']."' value='".$v['center']."'  class='center' /></td>
	  <td><input type='text' name='range_".$v['id']."' id='range_".$v['id']."' value='".$v['range']."'  class='range' /></td>
	  <td><div class=area_div><input type='hidden' name='area_".$v['id']."' id='area_".$v['id']."' value='".$v['area_id']."'  class='area' /> <script src='include/core/area_js.php?callback=set_area&input_id=area_".$v['id']."&id=".$v['area_id']."&level=4&output=select' id='area_".$v['id']."_area_js'></script> <span class=state id=area_".$v['id']."_state></span></div></td>
...
$sum=0;
$module['list']=admin_circle_get_list($pdo,self::$language,0,$module['module_name']);
