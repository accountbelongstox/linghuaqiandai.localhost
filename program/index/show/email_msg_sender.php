...
$page_size=min($page_size,100);
			...
			foreach($r as $v){
				...
				<td><a href='index.php?cloud=index.email_msg_detail&id=".$v['id']."' target=_blank class=title>".$v['title']."</a></td>
	...
	";	
		...
		$module['filter']="<select id='state' name='state'><option value='-1'>".self::$language['state']."</option><option value='' selected>".self::$language['all']."</option><option value='1'>".self::$language['email_msg_state'][1]."</option><option value='2'>".self::$language['email_msg_state'][2]."</option><option value='3'>".self::$language['email_msg_state'][3]."</option></select>";
		$module['list']=$list;
