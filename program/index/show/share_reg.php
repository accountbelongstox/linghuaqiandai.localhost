...
$module['share_content']="<a href='/index.php?cloud=".$r['url']."' target=_blank>".$r['title']."</a>";
$_GET['current_page']=(intval(@$_GET['current_page']))?intval(@$_GET['current_page']):1;
$page_size=self::$module_config[str_replace('::','.',$method)]['pagesize'];
$page_size=(intval(@$_GET['page_size']))?intval(@$_GET['page_size']):$page_size;
$page_size=min($page_size,100);

$sql="select * from ".$pdo->index_pre."share_reg where `share_id`=".$id;
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	...
	$sum_sql=str_replace("_share_reg and","_share_reg where",$sum_sql);
...
$sql=str_replace("_share_reg and","_share_reg where",$sql);
//echo($sql);
//exit();
$r=$pdo->query($sql,2);
$list='';
