...
$page=(isset($_GET['cloud']) && @$_GET['cloud']!='')?$_GET['cloud']:"index.index";
$t=explode(".",$page);
$menu=require("./program/".$t[0]."/menu.php");
//var_dump($menu);
$position=array();
//$position[]="<a href='index.php'><span id=user_position_icon>&nbsp;</span>".self::$config['web']['name']."</a>";
$position[]="<a href='index.php?cloud=index.user'>".self::$language['user_center']."</a>";

function user_position_get_name($key){
	if(!in_array($key,$_SESSION['user']['page'])){return false;}
	global $language;
	if(isset($language['pages'][$key]['name'])){return  "<a href='index.php?cloud=$key'>".$language['pages'][$key]['name'].'</a>';}	
	$temp=explode(".",$key);
