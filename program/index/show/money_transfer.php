<?php
		...
		$sql="select `transaction_password` from ".$pdo->index_pre."user where `username`='".$_SESSION['user']['username']."'";
		$r=$pdo->query($sql,2)->fetch(2);
		if($r['transaction_password']==''){echo ("<script>window.location.href='index.php?cloud=index.edit_user&field=transaction_password'</script>");return false;}
