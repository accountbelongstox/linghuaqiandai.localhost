<?php
		...
		$module['groups']=index::get_groups($pdo,0,"index.php?cloud=index.edit_function&program=".$program."&id=");
		$id=intval(@$_GET['id']);
		if($id==0){
			$sql="select `id` from ".$pdo->index_pre."group where `parent`=0 order by `sequence` desc limit 0,1";
			$r=$pdo->query($sql,2)->fetch(2);
			header("location:index.php?cloud=index.edit_function&id=".$r['id']);	
		}
		...
		foreach($stmt as $v){
			...
			foreach($language['functions'] as $key=>$v2){
				if(!(in_array($key,self::$config['unlogin_function_power']))){
					//echo $key.'<br/>';
					if(in_array($key,$function_power)){$checked="checked";}else{$checked='';}
					$module['list'].="<li><input class='checkbox' type='checkbox' id='{$key}' name='{$key}' $checked /> <b>".$language['functions'][$key]['description']."</b><br/>".self::$language['suggest'].": ".$language['functions'][$key]['power_suggest']."</li>";
				}
			}
