...
$sql="select * from ".$pdo->index_pre."recharge where `username`='".$_SESSION['user']['username']."'";
...
foreach($r as $v){
	...
	$operation='';
	if($v['state']==2){$operation=' <a href='.$v['id'].' class=inquiries_pay_state>'.self::$language['inquiry'].'</a> <span id=state_'.$v['id'].'></span>';	}
	...
	<td><span class=state>".self::$language['recharge_state'][$v['state']]." </span>  ".$operation."</td>
</tr>
";	
}
...
$sql="select sum(money) as c from ".$pdo->index_pre."recharge where `state`=4 and `username`='".$_SESSION['user']['username']."'";
...
$module['filter'].="<select id='state' name='state'><option value='-1'>".self::$language['state']."</option><option value='' selected>".self::$language['all']."</option><option value='1'>".self::$language['recharge_state'][1]."</option><option value='2'>".self::$language['recharge_state'][2]."</option><option value='3'>".self::$language['recharge_state'][3]."</option><option value='4'>".self::$language['recharge_state'][4]."</option></select>";
$module['list']=$list;
