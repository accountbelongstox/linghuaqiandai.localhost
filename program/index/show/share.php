...
$sql="select * from ".$pdo->index_pre."share";

$where="";
if($_GET['search']!=''){$where=" and (`title` like '%".$_GET['search']."%' or `username` like '%".$_GET['search']."%')";}
if(@$_GET['order']==''){
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	...
	//get sum visit reg 
	if($where==''){
		$sum_sql="select count(id) as c from ".$pdo->index_pre."share_visit";
		$r=$pdo->query($sum_sql,2)->fetch(2);
		$visit=$r['c'];
		$sum_sql="select count(id) as c from ".$pdo->index_pre."user where `introducer`!=''";
		$r=$pdo->query($sum_sql,2)->fetch(2);
		$reg=$r['c'];
...
foreach($r as $v){
	...
	<td><a href='/index.php?cloud=index.share&search=".urlencode($v['username'])."'  class=username>".$v['username']."</a></td>
	<td><a href='/index.php?cloud=".$v['url']."' target=_blank class=title>".$v['title']."</a></td>
	<td><a href='/index.php?cloud=index.share_visit&id=".$v['id']."' target=_blank class=visit>".$visit."</a></td>
	<td><a href='/index.php?cloud=index.share_reg&id=".$v['id']."' target=_blank class=reg>".$reg."</a></td>
