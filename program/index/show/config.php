		...
		function echo_html($language,$pdo,$key,$v){
			...
			switch ($key){
				...
				case ($key=='language' && $upkey=='web'):
					...
					$div='<div id=div_'.$key2.'><span class="m_label">'.$config_language[$key].'</span><span class="content"><select id='.$key2.' name='.$key2.'>'.$option.'</select><span id='.$key2.'_state ></span> <a href="/index.php?cloud=index.language_list&program=os_language">'.$language['edit'].'</a></span></div>';
					break;
				...
				case ($key=='authcode_push_mode' && $upkey=='web'):
					...
					$div='<div id=div_'.$key2.'><span class="m_label">'.$config_language[$key].'</span><span class="content"><select id='.$key2.' name='.$key2.' cloud_value='.$v.'>'.$option.'</select><span id='.$key2.'_state ></span></span></div>';
					break;
				...
				case ($key=='sms_api' && $upkey=='sms')://短信API选择
					$option='';
					foreach($language['smsapi_option'] as $kk=>$vv){
						$option.='<option value="'.$kk.'">'.$vv.'</option>';	
					}
					$div='<div id=div_'.$key2.'><span class="m_label">'.$config_language[$key].'</span><span class="content"><select id='.$key2.' name='.$key2.' cloud_value='.$v.'>'.$option.'</select><span id='.$key2.'_state ></span></span></div>';
					break;
				...
				default:
					...
					if($v===true || $v===false){
						...
						$div='<div id=div_'.$key2.'><span class="m_label">'.$config_language[$key].$other_content.'</span><span class="content"><select id='.$key2.' name='.$key2.'><option value="true" '.$true.'>'.$language['true'].'</option><option value="false" '.$false.'>'.$language['false'].'</option></select><span id='.$key2.'_state ></span></span></div>';
					}else{
						$div='<div id=div_'.$key2.'><span class="m_label">'.$config_language[$key].$other_content.'</span><span class="content"><input type="text" id="'.$key2.'" name="'.$key2.'" value=\''.$v.'\' /><span id='.$key2.'_state ></span></span></div>';
					}
					break;
		...
		$language=self::$language['config_language'];
			...
			foreach($config as $key=>$v){
				...
				if(is_array($v)){
					...
					$module['select'].='<div class="c_option">'.$language[$key]['this_name'].'</div>';
					$module['list'].='<fieldset style="display:none;" id='.$key.'>';
