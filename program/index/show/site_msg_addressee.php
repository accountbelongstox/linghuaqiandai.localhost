<?php
$module['cloud_table_name']=self::$language['pages'][str_replace("::",".",$method)]['name'];
...
if($_GET['search']!=''){$where=" and (`title` like '%".$_GET['search']."%' or `content` like '%".$_GET['search']."%' or `sender` like  '%".$_GET['search']."%')";}
$where.=" and `addressee_state`!='3' and `addressee`='".$_SESSION['user']['username']."'";
if(@$_GET['order']==''){
...
foreach($r as $v){
	...
	<td><a href='index.php?cloud=index.site_msg_detail&id=".$v['id']."' target=_blank class=title>".$v['title']."</a></td>
