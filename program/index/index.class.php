...
//=================================================================================================================	
	static function get_user_sub_ids($pdo,$id){
		$sql="select `id` from ".$pdo->index_pre."user where `manager`=$id or `user_group`=$id";
		$r=$pdo->query($sql,2);
		$ids='';
		foreach($r as $v){
			$ids.=$v['id'].',';	
...
//=================================================================================================================	
	static function check_user_power($pdo,$id){
		...
		if($r2['c']==1 || $_SESSION['user']['group_id']==1){
			...
			$groups=trim($groups,",");
			if($groups==''){$groups=0;}
			$sql="select count(*) as c from ".$pdo->index_pre."user where `group` in ($groups) and `id`='$id' or `user_group` in (".$_SESSION['user']['id'].") and `id`='$id'";
			$r=$pdo->query($sql,2)->fetch(2);
			return  $r['c'];
	...
	//获取用户短信数据
	static function get_user_msgs($pdo){
		$sql="select `state` from ".$pdo->index_pre."site_msg where `sender`='".$_SESSION['user']['username']."'";
		$r=$pdo->query($sql,2);
	...
	//获取用户表单
	static function get_user_tables($pdo){
		$groupid=$_SESSION['user']['group_id'];
		$sql="select `add_power`,`edit_power`,`admin_power`,`read_power`,`creater` from ".$pdo->table_pre."table where `creater`='".$_SESSION['user']['username']."'";
		$sql.=" or `edit_power` like '%".$groupid."|%'";
		$sql.=" or `admin_power` like '%".$groupid."|%'";
		$sql.=" or `read_power` like '%".$groupid."|%'";
		$sql.=" or `add_power` like '%".$groupid."|%'";
		$r=$pdo->query($sql,2);
		//exit($sql);
		$arr=array();
		$n1=0;//创建
		...
		//exit($sql);
		foreach ($r as $v) {
			$edit_power=explode("|", $v["edit_power"]);
			$admin_power=explode("|", $v["admin_power"]);
			$read_power=explode("|", $v["read_power"]);
			$add_power=explode("|", $v["add_power"]);
			if($v["creater"] == $_SESSION['user']['username']){
				$n1++;
			}
			if(in_array($groupid,$admin_power)){
				$n2++;
			}
			if(in_array($groupid,$edit_power)){
				$n3++;
			}
			if(in_array($groupid,$read_power)){
				$n4++;
			}
			if(in_array($groupid,$add_power)){
				$n5++;
			}
