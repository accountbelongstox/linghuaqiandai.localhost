...
class receive{
	...
	static function check_wid_power($pdo,$table_pre){
		...
		if($_SESSION['user']['username']!=$r['username'] && !in_array($_SESSION['user']['username'],$manager)){return false;}
		return $r['state'];
	}
	static function subscribe_notice_master($pdo,$table_pre,$toUsername,$fromUsername){
		...
		if($r['if_weixin']!=''){
			...
			"text":
			{
				 "content":"'.$nickname.' 刚关注了公众号：'.de_safe_str($r['name']).',聊天链接 http://'.$_SERVER['SERVER_NAME'].'/index.php?cloud=weixin.dialog&wid='.$toUsername.'&openid='.$fromUsername.'"
			}
		}  ';
	...
	static function get_access_token($pdo,$AppId,$AppSecret,$wid=''){
		...
		//if($wid!=''){if(isset($_SESSION['user'][$wid])){return $_SESSION['user'][$wid];}}
		if($wid==''){
		...
		/*以下代码为效验access_token是否过期*/
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, "https://api.weixin.qq.com/cgi-bin/menu/create?access_token=".$r['token']);
		...
		if(intval($tmpInfo['errcode']) == 42001){/*access_token过期*/
			return set_weixin_info_ReAccess_token($wid,$pdo);
		}else{
			return $r['token'];
		}
		...
		if($wid!=''){$_SESSION['user'][$wid]=$info->access_token;}
		if(isset($info->access_token)){return $info->access_token;}else{return false;}
		*/
	}

	static function get_weixin_token_Mysql($pdo,$AppId,$AppSecret,$wid=''){
...
<Articles>'.$list.'</Articles>';
	...
	function get_weixin_info($pdo,$table_pre,$username){
		/*得到微信的全部信息*/
		$re=return_table_info($pdo,$table_pre."account","all","username",$username,/*$all=*/true,/*$retype=*/"order by `id` desc");
		$re=echo_json($re);
		return $re;
	}
	...
	$sql="select count(id) as c from ".$table_pre."user where `openid`='".$openid."' and `wid`='".$wid."'";
	$r2=$pdo->query($sql,2)->fetch(2);
	if($r2['c']==0){
		$time=time();
