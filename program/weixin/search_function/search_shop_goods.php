...
function search_shop_goods($account,$pdo,$language,$key,$openid){
	...
	foreach($r as $v){
		if($i==9){
			...
			$data[$i]['url']=get_cloud_path().'index.php?cloud=shop.goods_list&current_page=2&search='.urlencode($key);
			$data[$i]['picurl']='';
			break;
		}
		$data[$i]['title']=$v['title'];
		$data[$i]['url']=get_cloud_path().'index.php?cloud=shop.goods&id='.$v['id'];
		$data[$i]['url']=get_cloud_path().'index.php?cloud=shop.goods&id='.$v['id'];
		$data[$i]['picurl']='';
		if($v['icon']!=''){$data[$i]['picurl']=get_cloud_path().'program/shop/img_thumb/'.$v['icon'];}
		if($i==0){
			$data[$i]['picurl']=get_cloud_path().'logo.png';	
			if($v['icon']!=''){$data[$i]['picurl']=get_cloud_path().'program/shop/img/'.$v['icon'];}
		}
		$i++;	
	}
	if($i==0){return '';}
