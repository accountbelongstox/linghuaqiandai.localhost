...
foreach($r as $v){
	...
	$auto_answer='';
	if($v['auto_answer']){
		$auto_answer=' <a href=/index.php?cloud=weixin.auto_answer_edit&wid='.$wid.'&id='.$v['auto_answer_id'].' class=edit target="_blank">'.self::$language['set'].'</a>';	
	}
...
require($t_path);
echo ' <div id="user_position_reset" style="display:none;"><a href="index.php"><span id=user_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href="index.php?cloud=index.user">'.self::$language['user_center'].'</a><a href="index.php?cloud=weixin.account_list">'.self::$language['pages']['weixin.account_list']['name'].'</a><a href="index.php?cloud=weixin.account_list&wid='.$wid.'">'.$w_name.'</a><span class=text>'.self::$language['pages']['weixin.diy_qr']['name'].'</span></div>';	
