...
foreach($r as $v){
	$v=de_safe_str($v);
	$del='';
	if($v['author']==$_SESSION['user']['username']){$del=" <a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a>";}
	$list.="<tr id='tr_".$v['id']."'>
	<td><input type='checkbox' name='".$v['id']."' id='".$v['id']."' class='id' /></td>
	<td><span class=key>".$v['key']."</span></td>
	<td><select id=like_".$v['id']." name=like_".$v['id']." cloud_value='".$v['like']."' class=like>".get_like_option(self::$language['like_state'])."</select></td>
  ...
  <td class=operation_td><a href='#' onclick='return update(".$v['id'].")'  class='submit'>".self::$language['submit']."</a> <a href='index.php?cloud=".$class.".auto_answer_edit&wid=".$wid."&id=".$v['id']."&type=".$v['input_type']."' class='edit'>".self::$language['edit']."</a>".$del." <span id=state_".$v['id']." class='state'></span></td>
</tr>
";	
}
...
echo ' <div id="user_position_reset" style="display:none;"><a href="index.php"><span id=user_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href="index.php?cloud=index.user">'.self::$language['user_center'].'</a><a href="index.php?cloud=weixin.account_list">'.self::$language['pages']['weixin.account_list']['name'].'</a><a href="index.php?cloud=weixin.account_list&wid='.$wid.'">'.$w_name.'</a><span class=text>'.self::$language['pages']['weixin.auto_answer_list']['name'].'</span></div>';	
