...
foreach($r as $v){
	...
	$location='';
	if($v['longitude']!=''){		
		$location='<a href=http://www.cloud.com/t_map.php?latitude='.$v['latitude'].'&longitude='.$v['longitude'].' target=_blank>'.self::$language['have'].'</a>';
	}
	...
	<td>&nbsp;<a href='index.php?cloud=index.admin_edit_user&username=".$v['username']."' target=_blank class=username>".$v['username']."</a></td>
	<td><span class=time>".get_time(self::$config['other']['date_style'],self::$config['other']['timeoffset'],self::$language,$v['time'])."</span></td>
  <td class=operation_td><a href='index.php?cloud=".$class.".dialog&wid=".$wid."&openid=".$v['openid']."' class='edit'>".self::$language['dialog']."</a>".$del." <span id=state_".$v['id']." class='state'></span></td>
</tr>
";	
}
...
echo ' <div id="user_position_reset" style="display:none;"><a href="index.php"><span id=user_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href="index.php?cloud=index.user">'.self::$language['user_center'].'</a><a href="index.php?cloud=weixin.account_list">'.self::$language['pages']['weixin.account_list']['name'].'</a><a href="index.php?cloud=weixin.account_list&wid='.$wid.'">'.$w_name.'</a><span class=text>'.self::$language['pages']['weixin.user_list']['name'].'</span></div>';	
