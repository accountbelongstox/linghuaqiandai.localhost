...
foreach(self::$language['input_type'] as $key=>$v){
	$input_select.='<a href="/index.php?cloud=weixin.auto_answer_add&wid='.$_GET['wid'].'&key='.urlencode($_GET['key']).'&type='.$key.'" id=input_type_'.$key.'>'.$v.'</a>';	
}
...
echo ' <div id="user_position_reset" style="display:none;"><a href="index.php"><span id=user_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href="index.php?cloud=index.user">'.self::$language['user_center'].'</a><a href="index.php?cloud=weixin.account_list">'.self::$language['pages']['weixin.account_list']['name'].'</a><a href="index.php?cloud=weixin.account_list&wid='.$wid.'">'.$w_name.'</a><a href="index.php?cloud=weixin.auto_answer_list&wid='.$wid.'">'.self::$language['pages']['weixin.auto_answer_list']['name'].'</a><span class=text>'.self::$language['pages']['weixin.auto_answer_add']['name'].'</span></div>';	

		$t_path='./templates/'.$m_require_login.'/'.$class.'/'.self::$config['program']['template_'.$m_require_login].'/'.$_COOKIE['cloud_device'].'/input_'.$type.'_add.php';
