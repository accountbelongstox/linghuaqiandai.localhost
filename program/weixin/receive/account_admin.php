<?php
		...
		if($act=='update'){
			...
			$_GET['sequence']=intval(@$_GET['sequence']);
  			$sql="update ".self::$table_pre."account set `state`='".$_GET['state']."',`sequence`='".$_GET['sequence']."' where `id`='$id'";
			if($pdo->exec($sql)){
		...
		if($act=='submit_select'){
			...
			foreach($_POST as $v){
				...
				$sql="update ".self::$table_pre."account set `state`='".$v['state']."' where `id`='".$v['id']."'";
