...
if($act=='add'){
	...
	if($account['AppSecret']==''){exit("{'state':'fail','info':'<span class=fail>AppSecret ".self::$language['is_null']."</span>'}");}
		...
		$r['access_token']=$_POST['cloud_weixin'][$account['wid']]['token'];
	...
	if($pdo->exec($sql)){
		...
		if(intval($_POST['auto_answer'])==1){
			$key='SCAN:'.$_POST['key'];
			$sql="insert into ".self::$table_pre."auto_answer (`wid`,`key`,`input_type`,`output_type`,`time`,`text`,`author`) values ('".$wid."','".$key."','text','text','".$time."','".self::$language['auto_answer'].' '.$key."','".$_SESSION['user']['username']."')";
			if($pdo->exec($sql)){
