...
<Content><![CDATA['.$r['text'].']]></Content>';
		...
		switch ($r['input_type']){
			case 'image':
				$data['picurl']=get_cloud_path().'program/weixin/image_thumb/'.$r[$r['input_type']];
				$data['url']=get_cloud_path().'program/weixin/image/'.$r[$r['input_type']].'?openid='.$openid;
				$data['title']='';
				$data['description']='';
				break;
			case 'voice':
				$data['picurl']=get_cloud_path().'program/weixin/img/voice.png';
				$data['url']=get_cloud_path().'index.php?cloud=weixin.index&show=dialog&type=voice&path='.$r[$r['input_type']].'&openid='.$openid;
				$data['title']='';
				$data['description']='';
				break;
			case 'video':
				$data['picurl']=get_cloud_path().'program/weixin/img/video.png';
				$data['url']=get_cloud_path().'index.php?cloud=weixin.index&show=dialog&type=video&path='.$r[$r['input_type']].'&openid='.$openid;
				$data['title']='';
				$data['description']='';
				break;
			case 'single_news':
				...
				$data['picurl']=get_cloud_path().'program/weixin/image_thumb/'.$r2['img'];
				$data['url']=$r2['url'];
				break;
			case 'news':
			...
			foreach($r2 as $v2){
				if($temp==0){
					$data[$temp]['picurl']=get_cloud_path().'program/weixin/image/'.$v2['img'];
				}else{
					$data[$temp]['picurl']=get_cloud_path().'program/weixin/image_thumb/'.$v2['img'];
				}
...
function weixin_add_dialog($pdo,$table_pre,$postObj){
	...
	switch($postObj->MsgType){
		...
		case 'image':		
			...
			$path=get_cloud_path().'/receive.php?target=weixin.index&act=save_image&id='.$pdo->lastInsertId();
...
function weixin_information_master($pdo,$table_pre,$language,$account,$v,$type){
	...
	if($account['if_email']!='' && $account['smtp_url']!=''  && $account['smtp_account']!=''  && $account['smtp_password']!=''){
		...
		if(($type=='receive' && $account['receive_if_email']==1) || ($type=='no_keyword' && $account['no_keyword_if_email']==1) || ($type=='no_search' && $account['no_search_if_email']==1)){
			...
			$content=$language['view'].$language['url'].':<a href="'.get_cloud_path().'index.php?cloud=weixin.dialog&wid='.$v->ToUserName.'&openid='.$v->FromUserName.'" target=_blank>'.get_cloud_path().'index.php?cloud=weixin.dialog&wid='.$v->ToUserName.'&openid='.$v->FromUserName.'</a>';
	...
	if($account['if_weixin']!='' && $account['AppId']!=''  && $account['AppSecret']!=''){
		...
		if(($type=='receive' && $account['receive_if_weixin']==1) || ($type=='no_keyword' && $account['no_keyword_if_weixin']==1) || ($type=='no_search' && $account['no_search_if_weixin']==1)){
			...
			$content=$language['view'].$language['url'].':'.get_cloud_path().'index.php?cloud=weixin.dialog&wid='.$v->ToUserName.'&openid='.$v->FromUserName;
...
function weixin_valid(){
	...
	if(weixin_checkSignature()){
		...
		//file_put_contents('temp/test.txt',$_GET["echostr"].'='.$_GET["signature"]);
	}
}	
