...
switch($type){
	...
	case 'image':
		...
		$data['picurl']=get_cloud_path().'program/weixin/image_thumb/'.$v;
		$data['url']=get_cloud_path().'program/weixin/image/'.$v.'?openid='.$openid;
	...
	case 'voice':
		...
		$data['url']=get_cloud_path().'index.php?cloud=weixin.index&show=dialog&type=voice&path='.$v.'&openid='.$openid;
	...
	case 'video':
		...
		$data['url']=get_cloud_path().'index.php?cloud=weixin.index&show=dialog&type=video&path='.$v.'&openid='.$openid;
		$v=self::weixin_voice_to_json($data,$openid);		
		$sql="insert into ".self::$table_pre."dialog (`wid`,`from`,`to`,`type`,`time`,`content`,`input_type`) values ('".$wid."','".$wid."','".$openid."','news','".$time."','".$v."','".$type."')";
		break;	
	case 'single_news':
		...
		$data['picurl']=get_cloud_path().'program/weixin/image_thumb/'.$img;
	...
	switch($type){
		...
		case 'news':
			...
			foreach($_POST as $k=>$v){
				...
				if($v['img']!=''){
					if($i==0){
						$data[$i]['picurl']=get_cloud_path().'program/weixin/image/'.$v['img'];
					}else{
						$data[$i]['picurl']=get_cloud_path().'program/weixin/image_thumb/'.$v['img'];
					}
				}else{
					$data[$i]['picurl']='';
				}
