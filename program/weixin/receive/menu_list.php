...
function crate_menu($language,$pdo,$table_pre,$wid){
	...
	//file_put_contents('temp/test.txt',$str);
	//exit;
	...
	if($tmpInfo['errmsg']!='ok'){
		return $tmpInfo['errcode'].$tmpInfo['errmsg'];
		}else{
			return true;
		}
...
function update_auto_answer($language,$pdo,$table_pre,$old_key,$r,$act=''){
	if(is_url($r['url']) && $old_key!=''){
		//echo $old_key;
		if(!is_url($old_key)){
			$sql="select * from ".$table_pre."auto_answer where `author`='cloud' and `key` like '%EventKey:".$old_key."|%'";
			$r=$pdo->query($sql,2)->fetch(2);
	...
	if(!is_url($r['url'])){
		...
		if($r2['id']==''){
			$sql="insert into ".$table_pre."auto_answer (`wid`,`key`,`input_type`,`output_type`,`time`,`text`,`author`) values ('".$_GET['wid']."','".$key."','text','text','".$time."','".$v."','cloud')";
		}else{
			if($act=='del'){
...
if($act=='add'){
	...
	$sql="select count(id) as c from ".self::$table_pre."menu where `url`='".$url."'";
	$r=$pdo->query($sql,2)->fetch(2);
	...
	if($pdo->exec($sql)){
		...
		//if($r!==true){exit("{'state':'fail','info':'<span class=fail>456".$r.self::$language['fail']." errmsg:".$r."</span>'}");}		
		//if($r!==true){exit("{'state':'fail','info':'<span class=fail>456".$r."</span>'}");}
		exit("{'state':'success','info':'<span class=success>".self::$language['success']."</span>','id':'".$pdo->lastInsertId()."'}");
	}else{
		exit("{'state':'fail','info':'<span class=fail>789".self::$language['fail']."</span>'}");
	}
}
if($act=='update'){
	...
	$sql="select count(id) as c from ".self::$table_pre."menu where `url`='".$_GET['url']."' and `id`!=".$_GET['id'];
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['c']!=0){exit("{'state':'fail','id':'url','info':'<span class=fail>".self::$language['url_or_key'].self::$language['already_exists']."</span>'}");}
