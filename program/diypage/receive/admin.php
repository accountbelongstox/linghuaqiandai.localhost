<?php
		...
		if($act=='update'){
			...
			$_GET['title']=safe_str(@$_GET['title']);
  			$sql="update ".self::$table_pre."page set `type`='".$_GET['type']."',`title`='".$_GET['title']."',`sequence`='".$_GET['sequence']."',`time`='$time',`editor`='$editor' where `id`='$id'";
			if($pdo->exec($sql)){
		...
		if($act=='submit_select'){
			...
			foreach($_POST as $v){
				...
				$sql="update ".self::$table_pre."page set `type`='".$v['type']."',`title`='".$v['title']."',`sequence`='".$v['sequence']."',`time`='$time',`editor`='$editor' where `id`='".$v['id']."'";
