...
foreach($r as $v){
	if($v['type']=='diy'){
		...
		$content='<a href="/index.php?cloud=diypage.show_page_list&type='.$v['type'].'&order=" class=title target='.self::$config['foot_six_grid_target'].'>'.$r2['name'].'</a>';
		$sql="select `id`,`title`,`link` from ".self::$table_pre."page where `type`='".$v['type']."' and `visible`=1 order by `sequence` desc limit 0,".$v['max'];
		//echo $sql.'<br />';
		$r3=$pdo->query($sql,2);
		foreach($r3 as $v3){
			if($v3['link']==''){$link='./index.php?cloud=diypage.show&id='.$v3['id'];}else{$link=$v3['link'];}
