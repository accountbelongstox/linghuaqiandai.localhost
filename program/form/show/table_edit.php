...
foreach($r as $v){
	if(!in_array($v['name'],self::$config['sys_field'])){
		$edit=" <a href='index.php?cloud=".$class.".field_edit&id=".$v['id']."' class='edit'>".self::$language['edit']."</a> <a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a>";
		$write_checkbox="<input class=write_able type=checkbox id=write_able_".$v['id']." value=".$v['write_able']." />";
	}else{$edit="";$write_checkbox='&nbsp;';}
	if($v['sequence'] < 0){
		$v_sequence="<input onkeyup=\"this.value=this.value.replace(/\D/g,'')\" onafterpaste=\"this.value=this.value.replace(/\D/g,'')\" maxlength=\"1\" class=sequence type=text id=sequence_".$v['id']." value=\"0\" />";
	}else{
		$v_sequence="<input onkeyup=\"this.value=this.value.replace(/\D/g,'')\" onafterpaste=\"this.value=this.value.replace(/\D/g,'')\" maxlength=\"1\" class=sequence type=text id=sequence_".$v['id']." value=".$v['sequence']." />";
	}
	$list.="<tr id='tr_".$v['id']."'>
	<td>".$v_sequence."</td>
