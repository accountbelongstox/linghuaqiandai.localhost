...
if(!in_array('0',$table_read_power)){
	if(!isset($_SESSION['user']['group_id'])){echo self::$language['without'].self::$language['view'].self::$language['power']; return false;}
	if(!in_array($_SESSION['user']['group_id'],$table_read_power)){echo self::$language['without'].self::$language['view'].self::$language['power']; return false;}	
}
...
require($t_path);
echo '<div id="visitor_position_reset" style="display:none;"><span id=current_position_text>'.self::$language['current_position'].'</span><a href="/index.php"><span id=visitor_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href="/index.php?cloud=form.data_show_list&table_id='.$table_id.'">'.$table_description.'</a>'.self::$language['detail'].'</div>';
