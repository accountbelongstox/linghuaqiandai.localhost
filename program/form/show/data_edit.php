...
if(!in_array('0',$table_edit_power)){
	if(!in_array($_SESSION['user']['group_id'],$table_edit_power)){
		echo self::$language['without'].self::$language['edit'].self::$language['power']; 
		return false;}	
}
$user_position_reset__= ' <div id="user_position_reset" style="display:none;"><a href="index.php"><span id=user_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href="index.php?cloud=index.user">'.self::$language['user_center'].'</a><a href="index.php?cloud=form.table_admin">'.self::$language['program_name'].'</a><a href="index.php?cloud=form.table_admin&id='.$table_id.'">'.$r['description'].'</a><a href="index.php?cloud=form.data_admin&table_id='.$table_id.'">'.self::$language['data'].self::$language['admin'].'</a><span class=text>'.self::$language['data'].self::$language['edit'].'</span></div>';	
