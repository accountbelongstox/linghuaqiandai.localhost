...
if(intval(@$_GET['id'])==0){exit('id is null');}
	...
	";	
		...
		$_SESSION['token'][$method]=get_random(8);$module['action_url']="/receive.php?token=".$_SESSION['token'][$method]."&target=".$method."&power=".$_GET['power'].'&id='.$_GET['id'];
		$name_count=0;
		$last_width=100;
...
$module['module_name']=str_replace("::","_",$method);
		...
		$sql="select `".$_GET['power']."_power`,`description` from ".self::$table_pre."table where `id`='".$_GET['id']."'";
		$r=$pdo->query($sql,2)->fetch(2);
		$module['ids']=$r[$_GET['power'].'_power'];
		
echo ' <div id="user_position_reset" style="display:none;"><a href="index.php"><span id=user_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href="index.php?cloud=index.user">'.self::$language['user_center'].'</a><a href="index.php?cloud=form.table_admin">'.self::$language['program_name'].'</a><a href="index.php?cloud=form.table_admin&id='.@$_GET['id'].'">'.$r['description'].'</a><span class=text>'.self::$language[$_GET['power']].self::$language['power'].'</span></div>';	
