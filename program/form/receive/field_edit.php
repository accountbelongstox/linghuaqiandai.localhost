...
if($act=='updateBefore'){
	...
	if($_POST['idAraay']=="clearAll"){/*全部清除*/
		$where=" where `table_id`=".$table_id;
	}else{/*部份清除*/
		$arr=explode("|",$_POST['idAraay']);
		if(count($arr)==0){
			exit('');
		}
		$where=" where `table_id`=".$table_id;
	...
	$sql="select `id` from ".self::$table_pre."field".$where;
	$r=$pdo->query($sql,2);
	foreach ($r as $value) {
		$sql="delete from ".self::$table_pre."field where `id`=".$value['id'];
		$pdo->exec($sql);
	}
