...
if($act=='update'){
	...
	//if(!in_array('0',$table_edit_power)){
		if(!in_array($_SESSION['user']['group_id'],$table_edit_power)){exit("{'state':'fail','info':'<span class=fail>".self::$language['without'].self::$language['edit'].self::$language['power']."</span>'}");}	
	//}
	...
	foreach($r as $v){
		...
		if($v['input_type']=='img'){
			if($_POST[$v['name']]!=''){
				if(file_exists('./temp/'.$_POST[$v['name']])){
					...
					@safe_unlink('./upload/form/img_thumb/'.$module['data'][$v['name']]);
					@safe_unlink('./upload/form/img/'.$module['data'][$v['name']]);
				}
			}else{
		...
		if($v['input_type']=='imgs'){
			...
			foreach($temp as $v2){
				if(!in_array($v2,$temp2)){
					@safe_unlink('./upload/form/imgs_thumb/'.$v2);
					@safe_unlink('./upload/form/imgs/'.$v2);
				}
			}
		...
		if($v['input_type']=='file'){
			if($_POST[$v['name']]!=''){
				if(file_exists('./temp/'.$_POST[$v['name']])){
					...
					@safe_unlink('./upload/form/file/'.$module['data'][$v['name']]);
				}
			}else{
				$_POST[$v['name']]=$module['data'][$v['name']];	
			}	
		...
		if($v['input_type']=='files'){
			...
			foreach($temp as $v2){
				if(!in_array($v2,$temp2)){
					//echo $v2;
					@safe_unlink('./upload/form/files/'.$v2);
				}
			}
	...
	$values=trim($values,',');
	$edit_time=time();
	$editor=(isset($_SESSION['user']['id']))?$_SESSION['user']['id']:0;
	...
	if($pdo->exec($sql)){
		...
		exit("{'state':'success','info':'<span class=success>".self::$language['submit'].self::$language['success']." ".$view.' <a href="/index.php?cloud=form.data_edit&table_id='.$table_id.'&id='.$id.'" class="edit">'.self::$language['continue'].self::$language['edit'].'</a>'."</span>'}");			
