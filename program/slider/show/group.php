...
function get_slider_style_option($config,$value=''){
	$path='./templates/0/slider/'.$config['program']['template_1'].'/pc/style/';
	$r=scandir($path);
	$option='';
	foreach($r as $v){
		if(is_dir($path.$v)){
...
foreach($r as $v){
  ...
  <td class=operation_td><a href='index.php?cloud=".$class.".show&id=".$v['id']."' target=_blank  class='view'>".self::$language['view']."</a><a href='#' onclick='return update(".$v['id'].")'  class='submit'>".self::$language['submit']."</a><a href='index.php?cloud=".$class.".img&id=".$v['id']."' class='edit'>".self::$language['edit']."</a><a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a> <span id=state_".$v['id']." class='state'></span></td>
</tr>
";	
}
