<?php
if(isset($_GET['cloud']) && $_GET['cloud']=='slider.show'){$id=intval($_GET['id']);}else{$id=intval($temp[1]);}
...
if($id>0){
	...
	$t_path='./templates/'.$m_require_login.'/'.$class.'/'.self::$config['program']['template_'.$m_require_login].'/'.$_COOKIE['cloud_device'].'/style/'.$module['style'].'/'.str_replace($class."::","",$method).'.php';
