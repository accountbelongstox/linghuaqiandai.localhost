				...
				if($r['receive']!='' && $r['answer']!=$_POST['answer']){
					if(self::$config['alert']['email_msg']){
						...
						if(is_email($email)){
							...
							email(self::$config,self::$language,$pdo,'cloud',self::$config['alert']['admin_email_account'],self::$config['web']['name'].' '.$_SERVER['HTTP_HOST'].self::$language['feedback_email_to_sender_title'],$content);
						}
					}
					if(self::$config['alert']['phone_msg']){
						...
						if(preg_match(self::$config['other']['reg_phone'],$phone)){
							//sms(self::$config,self::$language,$pdo,'cloud',self::$config['alert']['admin_phone_account'],self::$config['web']['name'].' '.$_SERVER['HTTP_HOST'].self::$language['feedback_email_to_sender_title'].':'.$_POST['answer']);
						}
					}
