...
if($act=='add'){	
	...
	if($pdo->exec($sql)){
		...
		if(self::$config['alert']['admin_phone_msg'] && preg_match(self::$config['other']['reg_phone'],self::$config['alert']['admin_phone_account'])){
			//sms(self::$config,self::$language,$pdo,'cloud',self::$config['alert']['admin_phone_account'],self::$config['web']['name'].' '.$_SERVER['HTTP_HOST'].self::$language['feedback_email_to_webmaster_title']);
		}
		if(self::$config['alert']['admin_email_msg'] && is_email(self::$config['alert']['admin_email_account'])){
			...
			email(self::$config,self::$language,$pdo,'cloud',self::$config['alert']['admin_email_account'],self::$config['web']['name'].' '.$_SERVER['HTTP_HOST'].self::$language['feedback_email_to_webmaster_title'],$content);	
		}
