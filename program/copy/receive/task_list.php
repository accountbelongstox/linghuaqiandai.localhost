...
if($act=='all_re_copy'){
	...
	if($pdo->exec($sql)){
		exit("{'state':'success','info':'<span class=success>".self::$language['success'].' '.self::$language['state']."=".self::$language['task_state'][0]." <a href=\"./index.php?cloud=copy.task_list&id=".$id."\" class=refresh>".self::$language['refresh']."</a></span>'}");
	}else{
