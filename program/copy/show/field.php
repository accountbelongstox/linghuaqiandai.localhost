...
echo '<div  style="display:none;" id="user_position_append"><a href="/index.php?cloud=copy.regular&id='.$id.'">'.$r['name'].'</a><span class=text>'.self::$language['pages']['copy.field']['name'].'</span></div>';
...
foreach($table as $v){
	...
	<select class='data_source_type' id='data_source_type' name='data_source_type' cloud_value='".$v3['data_source_type']."'>". get_data_source_type_option(self::$language['data_source_type_option'])."</select>
	...
	<div class=data_source_type_option_div id=data_source_type_2><span class=m_label_2>".self::$language['data_source_type_label'][2]."</span><span class=input_span_2><select id=data_source_2 name=data_source_2 cloud_value='".$v3['data_source_2']."'>".get_data_source_2()."</select> <span class=state_2></span></span></div>
	...
	<div class=data_source_type_option_div id=data_source_type_4><span class=m_label_2>".self::$language['data_source_type_label'][4]."</span><span class=input_span_2><select id=data_source_4 name=data_source_4 cloud_value='".$v3['data_source_4']."'>".get_data_source_4($pdo,$table)."</select> <span class=state_2></span></span></div>
	</td>
	<td><textarea id='extract_reg' name='extract_reg'>".$v3['extract_reg']."</textarea></td>
	<td class='data_type_td'>
	<select class='data_type' id='data_type' name='data_type' cloud_value='".$v3['data_type']."'>". get_data_type_option(self::$language['data_type_option'])."</select>
	<div class=data_type_option_div id=data_type_0>
	".self::$language['allow_html']."<select id='allow_html' name='allow_html' cloud_value='".$v3['allow_html']."' ><option value=0>".self::$language['no']."</option><option value=1>".self::$language['yes']."</option></select> <span class=state_2></span><br />
	...
	".self::$language['html_img_watermark']."<select id='html_img_watermark' name='html_img_watermark' cloud_value='".$v3['html_img_watermark']."' ><option value=0>".self::$language['no']."</option><option value=1>".self::$language['yes']."</option></select> <span class=state_2></span><br />
	</div>
	...
	".self::$language['data_type_img_imageMark']."<select id='data_type_img_imageMark' name='data_type_img_imageMark' cloud_value='".$v3['data_type_img_imageMark']."' ><option value=0>".self::$language['no']."</option><option value=1>".self::$language['yes']."</option></select> <span class=state_2></span><br />
