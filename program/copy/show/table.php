...
echo '<div  style="display:none;" id="user_position_append"><a href="/index.php?cloud=copy.regular&id='.$id.'">'.$r['name'].'</a><span class=text>'.self::$language['pages']['copy.table']['name'].'</span></div>';
...
foreach($table as $v){
	...
	<td><select id=multiple name=multiple cloud_value='".$v2['multiple']."'>".get_table_multiple_option(self::$language['table_multiple_option'])."</select></td>
