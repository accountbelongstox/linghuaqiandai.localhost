<?php
		...
		if($act=='update_visible'){
			...
			$sql="update ".self::$table_pre."img set `visible`='".$_GET['visible']."' where `id`='$id'";	
			$pdo->exec($sql);
			exit();
		}
		if($act=='update'){
			...
			$_GET['title']=safe_str(@$_GET['title']);
  			$sql="update ".self::$table_pre."img set `type`='".$_GET['type']."',`title`='".$_GET['title']."',`sequence`='".$_GET['sequence']."',`time`='$time',`editor`='$editor' where `id`='$id'";
			if($pdo->exec($sql)){
		...
		if($act=='del'){
			...
			if($pdo->exec($sql)){
				@safe_unlink("./program/image/img_thumb/".$r['src']);
				@safe_unlink("./program/image/img/".$r['src']);
		...
		if($act=='operation_visible'){
			...
			$ids=trim($temp,","); 
			$sql="update ".self::$table_pre."img set `visible`='".$_GET['visible']."' where `id` in ($ids)";
			$pdo->exec($sql);
		...
		if($act=='submit_select'){
			...
			foreach($_POST as $v){
				...
				$sql="update ".self::$table_pre."img set `type`='".$v['type']."',`title`='".$v['title']."',`sequence`='".$v['sequence']."',`time`='$time',`editor`='$editor' where `id`='".$v['id']."'";
