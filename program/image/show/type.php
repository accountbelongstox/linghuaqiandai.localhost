...
$module['module_name']=str_replace("::","_",$method);
		...
		function get_list($pdo,$language,$parent,$table_pre,$class){
			global $sum,$module;
			$obj=new $class($pdo);
			$sql="select * from ".$table_pre."img_type where `parent`='$parent'";
			...
			foreach($r as $v){
				...
				<td><a href=# id=icon_".$v['id']." class=icon><img src='/program/image/type_icon/".$v['id'].".png'></a></td>
			  <td><input type='text' name='sequence_".$v['id']."' id='sequence_".$v['id']."' value='".$v['sequence']."' class='sequence' /> <a href=/index.php?cloud=image.type_remark&id=".$v['id']." class=remark>".$language['remark']."</a></td>
	...
	";	
			$sql2="select count(id) as c from ".$table_pre."img_type where `parent`=".$v['id']."";
		...
		function get_name_width($pdo,$id,$table_pre){
			$sql="select `parent`,`id` from ".$table_pre."img_type where `id`='$id'";
			$v=$pdo->query($sql,2)->fetch(2);
			if($v['parent']==0){return 97;}	
			$sql="select `parent`,`id` from ".$table_pre."img_type where `id`='".$v['parent']."'";
			$v=$pdo->query($sql,2)->fetch(2);
