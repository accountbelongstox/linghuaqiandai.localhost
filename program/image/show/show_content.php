<?php
		...
		$id=$attribute['id'];
		if($id>0){
			$sql="select ".$attribute['field'].",`phone_content` from ".self::$table_pre."img where `id`='$id'";
			//echo $sql;
			$module=$pdo->query($sql,2)->fetch(2);
			$module['title']=de_safe_str(@$module['title']);
			if($_COOKIE['cloud_device']=='phone' && $module['phone_content']!=''){
			...
			$module['title']=self::$language['functions']['image.show_content']['description'];	
			$module['content']=self::$language['functions']['image.show_content']['description'].'( '.self::$language['edit_mode_click_the_right_attribute_for_set_here_data'].' )';
		}
