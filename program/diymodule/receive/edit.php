...
if($id>0){
	...
	$sql="update ".self::$table_pre."module set `width`='".$_POST['width']."',`height`='".$_POST['height']."',`title`='".$_POST['title']."',`title_visible`='".$_POST['title_visible']."',`content`='".$_POST['content']."',`time`='$time',`editor`='$editor' where `id`='$id'";
	if($pdo->exec($sql)){
		...
		if(is_file('./program/mall/config.php')){
			...
			$config['goods_update_time']=time();
			file_put_contents('./program/mall/config.php','<?php return '.var_export($config,true).'?>');
		}
		
		exit("{'state':'success','info':'<span class=success>&nbsp;</span><script>window.location.href='index.php?cloud=".self::$config['class_name'].".admin';</script>'}");
		}else{
