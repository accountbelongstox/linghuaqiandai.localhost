...
$module['module_name']=str_replace("::","_",$method);
			...
			$search=trim($search);
			$current_page=(intval(@$_GET['current_page']))?intval(@$_GET['current_page']):1;
...
$page_size=min($page_size,100);
			...
			$page_size=(intval(@$_GET['page_size']))?intval(@$_GET['page_size']):$page_size;
			$page_size=min($page_size,100);
			$sql="select `id`,`title`,`sequence`,`width`,`height`,`editor`,`time` from ".self::$table_pre."module";
			...
			$limit=" limit ".($current_page-1)*$page_size.",".$page_size;
				$sum_sql=$sql.$where;
				$sum_sql=str_replace(" `id`,`title`,`sequence`,`width`,`height`,`editor`,`time` "," count(id) as c ",$sum_sql);
				$sum_sql=str_replace("_module and","_module where",$sum_sql);
				//echo $sum_sql;
				$r=$pdo->query($sum_sql,2)->fetch(2);
				$sum=$r['c'];
			$sql=$sql.$where.$order.$limit;
			$sql=str_replace("_module and","_module where",$sql);
			//echo($sql);
			//exit();
			$r=$pdo->query($sql,2);
			$list='';
			foreach($r as $v){
			  ...
			  <td class=operation_td><a href='#' onclick='return update(".$v['id'].")'  class='submit'>".self::$language['submit']."</a> <a href='index.php?cloud=".$class.".edit&id=".$v['id']."' class='edit'>".self::$language['edit']."</a> <a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a> <span id=state_".$v['id']." class='state'></span></td>
			</tr>
	";	
			}
		...
		$module['page']=cloudDigitPage($sum,$current_page,$page_size,'#'.$module['module_name'],self::$language['page_template']);
