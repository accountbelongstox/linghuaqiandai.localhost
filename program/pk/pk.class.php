...
//=======================================================================================================
	...
	function get_type_names($pdo,$id,$target,$title=''){
		$names='<a href="/index.php?cloud=pk.show_page_list&type='.$id.'" class=parent target='.$target.'>'.$title.'</a>';			
		...
		foreach($r as $v){
			$names.='<a href="/index.php?cloud=pk.show_page_list&type='.$v['id'].'" id="item_'.$v['id'].'" target='.$target.'>'.$v['name'].'<span class=show_sub>&nbsp;</span></a>';
			...
			foreach($r2 as $v2){
				$names.='<a href="/index.php?cloud=pk.show_page_list&type='.$v2['id'].'" id="item_'.$v2['id'].'" target='.$target.'>'.$v2['name'].'<span class=show_sub>&nbsp;</span></a>';
				...
				foreach($r3 as $v3){
					$names.='<a href="/index.php?cloud=pk.show_page_list&type='.$v3['id'].'" id="item_'.$v3['id'].'" target='.$target.'>'.$v3['name'].'</a>';
				}
				$names.='</div>';
			}
			$names.='</div>';
	...
	function get_pk_value_input($pdo,$thead,$item_id){
		...
		foreach($thead as $vv){
			...
			$input.='<td class=object object_id='.$vv.' item_id='.$item_id.'><input type=checkbox cloud_value='.$v3['checked'].' class=pk_checkbox /><input type=text value="'.de_safe_str($v3['value']).'" class=pk_value /></td>';
		}
		return $input;
	}
