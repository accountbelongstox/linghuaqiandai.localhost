...
$sql="select `id`,`name`,`username`,`style`,`state` from ".self::$table_pre."pk where `id`=".$pk_id;
$pk=$pdo->query($sql,2)->fetch(2);
if($pk['id']==''){echo 'pk_id err';return false;}
if($pk['state']==0){header('location:./index.php');exit;}
$module['style']=$pk['style'];
$module['action_url']="/receive.php?target=".$method."&id=".$pk_id;
$module['cloud_table_name']=$pk['name'].' '.self::$language['functions'][str_replace("::",".",$method)]['description'];
...
foreach($r as $v){
	...
	foreach($t2 as $v2){
		...
		foreach($t3 as $v3){
			$item_deep=3;
			$input3=self::get_pk_value($pdo,$thead,$v3['id']);
		...
		$input2=self::get_pk_value($pdo,$thead,$v2['id']);
	...
	$input=self::get_pk_value($pdo,$thead,$v['id']);
	if($module['list_2']!=''){}
