...
function get_pk_style_option($config,$value=''){
	$path='./templates/0/pk/'.$config['program']['template_1'].'/pc/style/';
	$r=scandir($path);
	$option='';
	foreach($r as $v){
		if(is_dir($path.$v)){
...
$module['style_option']=get_pk_style_option(self::$config);
...
$sql="select * from ".self::$table_pre."pk where `username`='".$_SESSION['user']['username']."'";
...
$limit=" limit ".($current_page-1)*$page_size.",".$page_size;
	...
	$sum_sql=str_replace("_pk and","_pk where",$sum_sql);
	//echo $sum_sql;
	$r=$pdo->query($sum_sql,2)->fetch(2);
	$sum=$r['c'];
$sql=$sql.$where.$order.$limit;
$sql=str_replace("_pk and","_pk where",$sql);
//exit();
$r=$pdo->query($sql,2);
$list='';
foreach($r as $v){
	...
	<td><select id='style_".$v['id']."' name='style_".$v['id']."' class=style>".get_pk_style_option(self::$config,$v['style'])."</select></td>
	...
	<td>".$v['visit']."</td>
  <td class=operation_td><a href='#' onclick='return update(".$v['id'].")'  class='submit'>".self::$language['submit']."</a><a href='index.php?cloud=".$class.".item&pk_id=".$v['id']."' class='set'>".self::$language['pages']['pk.item']['name']."</a><a href='index.php?cloud=".$class.".object&pk_id=".$v['id']."' class='set'>".self::$language['pages']['pk.object']['name']."</a><a href='index.php?cloud=".$class.".value&pk_id=".$v['id']."' class='set'>".self::$language['pages']['pk.value']['name']."</a><a href='index.php?cloud=".$class.".show&pk_id=".$v['id']."' target=_blank  class='view'>".self::$language['view']."</a><a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a> <span id=act_state_".$v['id']." class='act_state'></span></td>
</tr>
";	
}
