...
if($pk['username']!=$_SESSION['user']['username']){exit("{'state':'fail','info':'<span class=fail>pk_id power err</span>'}");}
...
if($act=='add'){
	...
	$sql="insert into ".self::$table_pre."item (`parent`,`name`,`sequence`,`state`,`pk_id`,`username`) values ('".$_GET['parent']."','".$_GET['name']."','".$_GET['sequence']."','".$_GET['state']."','".$pk_id."','".$_SESSION['user']['username']."')";
	if($pdo->exec($sql)){
