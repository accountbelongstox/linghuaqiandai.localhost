...
if($r['state']!=0){header('location:../../index.php?cloud=scanpay.pay&id='.$id.'&p_id='.$p_id);exit('state!=0');}
if($r['operator']!=$_SESSION['user']['username']){exit('no power this p_id');}
if($r['time']<time()-60){
...
$pdo->exec($sql);
header('location:../../index.php?cloud=scanpay.pay&id='.$id.'&p_id='.$p_id);exit;
