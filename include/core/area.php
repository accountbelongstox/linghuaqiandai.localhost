...
class area_select{
	...
	function get_select($id){
		...
		$list='';
			...
			if($temp!=''){
				if($level==2){$level_name=$language['city'];}
				if($level==3){$level_name=$language['county'];}
				if($level==4){$level_name=$language['twon'];}
				if($level==5){$level_name=$language['village'];}
				if($level==6){$level_name=$language['group'];}
	...
	function get_level1(){
		...
		return "<select class='{$input_id}_select_change'  autocomplete='off'><option value='0'>-".$language['province']."-</option>$list</select>";
	}	
}
