<div id=<?php echo $module['module_name'];?>  class="portlet light" cloud-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		$("#<?php echo $module['module_name'];?> .name").css('width','60%');
        ...
        $("[cloud-table] tbody tr").each(function(index, element) {
    ...
    function add(){
        ...
        if(urlNew.prop('value')==''){$("#<?php echo $module['module_name'];?> #url_new").html('<?php echo self::$language['please_input']?><?php echo self::$language['url']?>');urlNew.focus();return false;}	
        ...
        $.get('<?php echo $module['action_url'];?>&act=add',{parent:parentNew.prop('value'),name:nameNew.prop('value'),url:urlNew.prop('value'),sequence:sequenceNew.prop('value'),visible:visible}, function(data){
            //alert(data);
    ...
    function update(id){
        ...
        $.get('<?php echo $module['action_url'];?>&act=update',{parent:parent.prop('value'),name:name.prop('value'),url:url.prop('value'),sequence:sequence.prop('value'),id:id,visible:visible}, function(data){
            //alert(data);
            try{v=eval("("+data+")");}catch(exception){alert(data);}
	...
	#<?php echo $module['module_name'];?>_table .cloud_thead_span{ width:25%;}
    #<?php echo $module['module_name'];?>_table .name{width:5rem !important;}
    ...
    <div class=table_scroll><table class="table table-striped table-bordered table-hover dataTable no-footer"  role="grid"  id="<?php echo $module['module_name'];?>_table" style="width:100%" cellpadding="0" cellspacing="0">
         <thead>
            <tr>
                ...
                <td width="20%"><?php echo self::$language['url_or_key']?></td>
        ...
        <tbody>
            <tr id="<?php echo $module['module_name'];?>_new">
                ...
                <td ><input type="text" name="name_new" id="name_new" class='name' style="width:60%;" /></td>
