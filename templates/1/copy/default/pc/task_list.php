<div id=<?php echo $module['module_name'];?>  class="portlet light <?php echo $module['module_name'];?>" cloud-module="<?php echo $module['module_name'];?>" align=left >
	...
	function get_frist_page_task(){
        ...
        $.get('<?php echo $module['action_url'];?>&act=get_frist_page_task',function(data){
            ...
            //alert(data);
			try{v=eval("("+data+")");}catch(exception){alert(data);}
			$("#get_frist_page_task_a").next().html('<?php echo self::$language['match'];?>:'+v.sum+'<?php echo self::$language['bar'];?> <?php echo self::$language['new_task']?>:'+v.add+' <a href="/index.php?cloud=copy.task_list&id=<?php echo $_GET['id'];?>" class=refresh><?php echo self::$language['refresh'];?></a>');
        });
         return false;	
	}
    </script>
    ...
    #<?php echo $module['module_name'];?> .title{line-height:50px; vertical-align:middle; text-align:left; display:block; overflow:hidden;}
...
<div id="<?php echo $module['module_name'];?>_html" class="<?php echo $module['module_name'];?>_html" cloud-table="1">
    ...
    <div class="filter"><a href="#" id=get_frist_page_task_a onclick="get_frist_page_task()" class="add"><?php echo self::$language['get_frist_page_task']?></a> <span></span>  
       &nbsp; &nbsp; <a href="#" id=get_all_task_a onclick="get_all_task()" class="add"><?php echo self::$language['get_all_task']?></a> <span id=get_all_task_a_state></span>  
        ...
        &nbsp; &nbsp; <a href="/index.php?cloud=copy.task&regular_id=<?php echo $_GET['id'];?>" class="open"><?php echo self::$language['open_copy_panel']?></a> <span id="open_copy_panel_state"></span>
