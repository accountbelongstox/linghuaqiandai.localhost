<div id=<?php echo $module['module_name'];?>  class="portlet light" cloud-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		...
		$('.icon').click(function(){
			...
			//cloud_//alert(replace_img);
			$("#fade_div").css('display','block');
			$("#set_cloud_iframe_div").css('display','block');
			$("#cloud_iframe").attr('src','index.php?cloud=index.replace_file&path='+replace_file+'&width=256&height=256');
			return false;	
		});
    });
    ...
    function update(id){
        ...
        manager=$("#<?php echo $module['module_name'];?> #manager_"+id);
        ...
        if(manager.prop('value')==''){$("#<?php echo $module['module_name'];?> #state_"+id).html('<?php echo self::$language['please_input']?><?php echo self::$language['manager']?>');manager.focus();return false;}	
        
        $("#<?php echo $module['module_name'];?> #state_"+id).html('<span class=\'fa fa-spinner fa-spin\'></span>');
        $.get('<?php echo $module['action_url'];?>&act=update',{parent:parent.prop('value'),name:name.prop('value'),sequence:sequence.prop('value'),manager:manager.prop('value'),id:id,visible:visible}, function(data){
            //alert(data);
            try{v=eval("("+data+")");}catch(exception){alert(data);}
    ...
    function subimt_select(){
        ...
        for(id in ids){
            ...
            obj[id]['manager']=$("#<?php echo $module['module_name'];?> #manager_"+ids[id]).prop('value');
    ...
    #<?php echo $module['module_name'];?>_table .name{float:right; width:6rem !important;}
    ...
    <div class=table_scroll><table class="table table-striped table-bordered table-hover dataTable no-footer"  role="grid"  id="<?php echo $module['module_name'];?>_table" style="width:100%" cellpadding="0" cellspacing="0">
         <thead>
            <tr>
                ...
                <td width="4%"><?php echo self::$language['visible']?></td>
                <td><?php echo self::$language['manager']?></td>
