<div id=<?php echo $module['module_name'];?>  class="portlet light <?php echo $module['module_name'];?>" cloud-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		$("#<?php echo $module['module_name'];?> .username").blur(function(){
			...
			if($(this).val()!=''){
				$.post('<?php echo $module['action_url'];?>&act=require&username='+$(this).val(), function(data){
					try{v=eval("("+data+")");}catch(exception){alert(data);}
					$("#<?php echo $module['module_name'];?> .balance").html(v.money);
				});
			}
		});
        ...
        $("#<?php echo $module['module_name'];?> .submit").click(function(){
			...
			$.post('<?php echo $module['action_url'];?>&act=add',obj, function(data){
				...
				if(v.state=='fail'){
					...
					$("#<?php echo $module['module_name'];?> .username").val('');
					$("#<?php echo $module['module_name'];?> .balance").html('');
				}
...
<div id="<?php echo $module['module_name'];?>_html">
    	...
    	<div class=line><span class="m_label"><?php echo self::$language['balance']?></span><span class="input balance"> <?php echo $module['money']?></span></div>        
    	<div class=line><span class=m_label><?php echo self::$language['debit']?><?php echo self::$language['amount']?></span><span class=input> <input type="text" class=money value=""  /> <span class=state></span></span></div>        
    	<div class=line><span class=m_label><?php echo self::$language['debit']?><?php echo self::$language['reason']?></span><span class=input> <input type="text" class=reason value=""  /> <span class=state></span></span></div>        
