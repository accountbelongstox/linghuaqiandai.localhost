...
$(document).ready(function(){
	...
	$("#<?php echo $module['module_name'];?> .submit").click(function(){
		...
		if(!reg_phone.test($("#<?php echo $module['module_name'];?>_html #phone").val())){	  
			$(this).next().html('<span class=fail><?php echo self::$language['phone']?><?php echo self::$language['pattern_err']?></span>');
			$(this).focus();
			return false;
		}
		...
		$.post('<?php echo $module['action_url'];?>&act=update',{phone:$("#<?php echo $module['module_name'];?> #phone").val(),authcode:$("#<?php echo $module['module_name'];?> #authcode").val()}, function(data){
			//alert(data);
			try{v=eval("("+data+")");}catch(exception){alert(data);}				
			
			$("#<?php echo $module['module_name'];?> .submit").next().html(v.info);	
			if(v.state=='success'){alert('<?php echo self::$language['success']?>');window.location.href='/index.php?cloud=index.user';return false;}
		});
...
<div id="<?php echo $module['module_name'];?>_html" class="<?php echo $module['module_name'];?>_html" align="left">

<div class="portlet-title" style="padding-bottom: 20px;">
<div class="caption">修改手机号码</div>
</div>
<div class=<?php echo $module['module_name'];?>_div>
	  <div class=line><span class="m_label"><?php echo self::$language['phone'];?>：</span><span class=value><input  type="text" name="phone" id="phone" value="<?php echo $module['phone']?>"  /> <span class=state></span></span></div>
