<div id=<?php echo $module['module_name'];?>  class="portlet light <?php echo $module['module_name'];?>" cloud-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){  
		...
		$(".add").click(function(){
			...
			if(!module_name){cloud_alert('<?php echo self::$language['select_please'];?><?php echo self::$language['module'];?>');}
			...
			$.post("<?php echo $module['action_url'];?>&act=add_module", { module_name:module_name},function(data){
					...
					if(v.state=='success'){
						...
						url=url.replace(/\|\|\|/g,'&');
						window.location.href='index.php?cloud='+url;	 
					}
			});
			return false;	
		});
		...
		url='index.php?cloud='+url+'&edit_page_layout=true';
		url=url.replace(/\|\|\|/g,'&');	
