...
<script src="/plugin/datePicker/index.php"></script>
    ...
    $(document).ready(function(){
		...
		if(isWeiXin()){
			$("#<?php echo $module['module_name'];?> #online_div .payment").each(function(index, element) {
                if($(this).attr('payment')=='alipay_wap' || $(this).attr('payment')=='scanpay_alipay'){$(this).css('display','none');}
            });
		}else{
			$("#<?php echo $module['module_name'];?> #online_div .payment").each(function(index, element) {
        ...
        $("#submit_offline").click(function(){
			if(($("#pay_info").val()=="<?php echo $module['pay_info'];?>" || $("#pay_info").val()=='') && $("#pay_photo").val()==''){cloud_alert("<?php echo self::$language['pay_photo'];?>/<?php echo self::$language['pay_info'];?> <?php echo self::$language['whole_is_null'];?>");return false;}
	...
	#offline_div{  padding:10px; border:1px #ccc solid; border-radius:3px;}
	#offline_div #offlie_info{ display:inline-block;  overflow:hidden;}
	#online_div .payment{ display:inline-block; vertical-align:top;white-space:nowrap; overflow:hidden; width:45%; padding-right:4%; text-align:center; overflow:hidden; margin-bottom:2rem; }
    ...
    <tr ><td width="20%" align="right"><span><?php echo self::$language['recharge'];?><?php echo self::$language['amount'];?></span></td>
    <td width="80%">
    <input type="text" id="money" value="<?php echo @$_GET['money']?>"><span id=money_state></span>
    </td></tr>

    <tr id="offline_tr"><td width="20%" align="right"><?php echo self::$language['offline_payment']?></td>
    <td width="80%">
	<div id=offline_div>
    ...
    <span class=m_label><?php echo self::$language['pay_info'];?></span><input type="text" id='pay_info' name='pay_info' value="<?php echo $module['pay_info'];?>" ><br />
    <br />
