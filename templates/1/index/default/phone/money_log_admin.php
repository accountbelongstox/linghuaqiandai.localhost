...
<div id=<?php echo $module['module_name'];?>  class="portlet light" cloud-module="<?php echo $module['module_name'];?>" align=left >
	...
	function show_username(){
		if($("#usernmae").val()!='' && $("#username").val()!='<?php echo self::$language['username']?>'){
			window.location.href='/index.php?cloud=index.money_log_admin&username='+$("#username").val();
		}else{
			cloud_alert('<?php echo self::$language['please_input']?><?php echo self::$language['username']?>');
    ...
    #<?php echo $module['module_name'];?>_table .before_money{width:90px;display:inline-block;overflow:hidden;}
    ...
    <?php echo $module['filter']?>
        ...
        <div class=top_div><input type="text" name="username" id="username" value="<?php echo @$_GET['username'];?>"  placeholder="<?php echo self::$language['username']?>" /> <a href=# onclick="return show_username();" class="inquiry"><?php echo self::$language['inquiry']?></a><br /><?php echo $module['current_program'];?> <?php echo self::$language['sum']?>: +<?php echo $module['add']?> &nbsp; &nbsp; <?php echo $module['deduction']?></div>

    
    <div class=table_scroll><table class="table table-striped table-bordered table-hover dataTable no-footer"  role="grid" id="<?php echo $module['module_name'];?>_table_bak" style="width:100%" cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                ...
                <td ><?php echo self::$language['before_operation']?></td>
