<div id=<?php echo $module['module_name'];?>  class="portlet light" cloud-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){ 
		...
		if(get_param('program')==''){$("#program_list").css('display','block');$("[cloud-table] table").css('display','none');}
    ...
    function update(id){
		...
		//
        $.post('<?php echo $module['action_url'];?>&act=update',{id:id,head:head,left:left,right:right,full:full,bottom:bottom,phone:phone,layout:layout,target:target,tutorial:tutorial}, function(data){
			//cloud_alert(data);
                        try{v=eval("("+data+")");}catch(exception){alert(data);}
...
$.get('<?php echo $module['action_url'];?>&act=del',{id:id}, function(data){
	...
	#program_list a{ display:inline-block; vertical-align:top; width:33%; text-align:center; }
	#program_list a:hover{}
	#program_list a img{ width:60%; border:none; border-radius:50%; background:<?php echo $_POST['cloud_user_color_set']['nv_2']['background']?>;}
	#program_list a img:hover{ background:<?php echo $_POST['cloud_user_color_set']['nv_2_hover']['background']?>;}
	[cloud-table] .url{ display:inline-block;width:150px; overflow:hidden;}
	[cloud-table] .module_list{ display:inline-block;width:450px; overflow:hidden;}
	[cloud-table] .module_list input{ display:inline-block;width:330px; margin-left:5px; overflow:hidden; margin:5px;}
 	[cloud-table] .layout{ display:inline-block;width:100px; overflow:hidden;}
	[cloud-table] .target{ display:inline-block;width:100px; overflow:hidden;}
	[cloud-table] .tutorial{ display:inline-block;width:100px; overflow:hidden;}
