...
/**
 * Form AND/OR query array using String_new::tokenize to separate
 ...
 */
	protected function _connectedLike($value, $field, $fieldName) {
		$or = array();
		$orValues = String_new::tokenize($value, $field['connectorOr']);
		foreach ($orValues as $orValue) {
			$andValues = String_new::tokenize($orValue, $field['connectorAnd']);
			$and = array();
