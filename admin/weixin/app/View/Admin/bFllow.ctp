...
<div class="clearfix form-actions">
	<div class="col-xs-12 col-sm-9 col-sm-offset-3">
		...
		<script type="text/javascript">
			function check_parent_form(e){
				var form_=location_form(e,0);
				if(form_){
					var obj={};
					inputs=form_.find('*');
					for (var i = inputs.length - 1; i >= 0; i--) {
						var id_=$(inputs[i]).attr("id");
						if(id_){
							obj[id_]=$(inputs[i]).val();
						}
					}
					$.ajax({
	                cache: true,
			...
			function location_form(e,n){
				...
				if($(e).parent().is('form')){
					...
					n++;
					return location_form($(e).parent(),n);
				}
			}
		</script>
