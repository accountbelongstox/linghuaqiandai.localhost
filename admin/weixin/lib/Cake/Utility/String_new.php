 ...
 * Example: `String_new::insert(':name is :age years old.', array('name' => 'Bob', '65'));`
 ...
 * - clean: A boolean or array with instructions for String_new::cleanInsert
 *
 ...
 */
	public static function insert($str, $data, $options = array()) {
		...
		$data = (array)$data;
		if (empty($data)) {
			return ($options['clean']) ? String_new::cleanInsert($str, $options) : $str;
		}
...
/**
 * Cleans up a String_new::insert() formatted string with given $options depending on the 'clean' key in
 * $options. The default method used is text but html is also available. The goal of this function
 * is to replace all whitespace and unneeded markup around placeholders that did not get replaced
 * by String_new::insert().
 *
 * @param string $str
 * @param array $options
 * @return string
 * @see String_new::insert()
 */
		...
		switch ($clean['method']) {
			case 'html':
				...
				if ($clean['andText']) {
					...
					$str = String_new::cleanInsert($str, $options);
				}
				break;
			case 'text':
				$clean = array_merge(array(
