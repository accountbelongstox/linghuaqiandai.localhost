 ...
 */
	public function text($width = 72) {
		...
		if (!empty($description)) {
			$out[] = String_new::wrap($description, $width);
			$out[] = '';
		}
		...
		if (!empty($subcommands)) {
			...
			foreach ($subcommands as $command) {
				$out[] = String_new::wrap($command->help($max), array(
					'width' => $width,
		...
		if (!empty($options)) {
			...
			foreach ($options as $option) {
				$out[] = String_new::wrap($option->help($max), array(
					'width' => $width,
					'indent' => str_repeat(' ', $max),
					'indentAt' => 1
				));
		...
		if (!empty($arguments)) {
			...
			foreach ($arguments as $argument) {
				$out[] = String_new::wrap($argument->help($max), array(
					'width' => $width,
					'indent' => str_repeat(' ', $max),
					'indentAt' => 1
				));
		...
		if (!empty($epilog)) {
			$out[] = String_new::wrap($epilog, $width);
			$out[] = '';
		}
		return implode("\n", $out);
	}
