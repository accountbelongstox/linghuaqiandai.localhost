...
<div style="text-align:center; font-size:200px; padding-top:100px;"><i class='fa fa-spinner fa-spin' ></i></div>
<?php 
...
if($_REQUEST['state'] != $_SESSION['oauth_wx']['state'] || @$_GET['code']==''){
 exit('授权出错,<a href=../../index.php?index.user>返回</a>'); 
}
...
if (!isset($r['openid'])) {
	...
	exit('<div style=" text-align:center; line-height:300px; font-size:40px;">授权出错,<a href=../../index.php?index.user>返回</a></div>');         
}
$open_id=$r['openid'];
...
//=============================================================================================================================== start	
	...
	if(oauth_login($pdo,$language,$config)==false){
		...
		//exit('<a href=index.php?cloud=index.oauth_bind_option#index_oauth_bind_option>index.php?cloud=index.oauth_bind_option#index_oauth_bind_option</a>');
		//var_dump($_SESSION['oauth']);exit;
		exit("<script>window.location.href='../../index.php?cloud=index.oauth_bind_option#index_oauth_bind_option'</script>");
		//header('location:./index.php?cloud=index.oauth_bind_option#index_oauth_bind_option');
