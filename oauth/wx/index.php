...
$url='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$oauth_config['appid'].'&redirect_uri='.urlencode('http://'.$_SERVER['SERVER_NAME'].'/'.$oauth_config['callback']).'&response_type=code&scope='.$oauth_config['scope'].'&state='.$_SESSION['oauth_wx']['state'].'#wechat_redirect';

if(!isset($_COOKIE['cloud_device']) || $_COOKIE['cloud_device']=='phone' ){
	header("Location:$url");
	//echo $url;
	exit();
}

$url='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$oauth_config['appid'].'&redirect_uri='.urlencode('http://'.$_SERVER['SERVER_NAME'].'/'.$oauth_config['qr_callback']).'&response_type=code&scope='.$oauth_config['scope'].'&state='.$_SESSION['oauth_wx']['state'].'#wechat_redirect';
